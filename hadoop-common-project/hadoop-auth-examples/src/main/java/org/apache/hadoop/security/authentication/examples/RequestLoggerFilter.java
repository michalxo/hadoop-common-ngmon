...
public class RequestLoggerFilter implements Filter {
  private static /* LoggerLOG=LoggerFactory.getLogger(RequestLoggerFilter.class); */
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (!LogGlobal.isDebugEnabled()) {
      ...
      try {
        ...
        LOG.xrequestgetresqu(String.valueOf(xRequest.getResquestInfo().toString())).tag("methodCall").debug();
        filterChain.doFilter(xRequest, xResponse);
      }
      finally {
        /* LOG.debug(xResponse.getResponseInfo().toString()) */
        LOG.xresponsegetresp(String.valueOf(xResponse.getResponseInfo().toString())).tag("methodCall").debug();
      }
    }
  }
