...
public class IdUserGroup {
  static final /* LogLOG=LogFactory.getLog(IdUserGroup.class); */
  ...
  public IdUserGroup(Configuration conf) throws IOException {
    ...
    if (updateTime < TIMEOUT_MIN) {
      ...
      LOG.user_configured_user_account_update_time().info();
      timeout = TIMEOUT_MIN;
    } else {
      timeout = updateTime;
    }
  ...
  private static void reportDuplicateEntry(final String header,
      ...
      LOG.headerkey(header, key.toString()).warn();
  }
  ...
  public static void updateMapInternal(BiMap<Integer, String> map, String mapName,
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        LOG.add_map(mapName, String.valueOf(nameId[0]), String.valueOf(nameId[1])).debug();
      ...
      LOG.updated_map_size(mapName, String.valueOf(map.size())).tag("methodCall").info();
      
    } catch (IOException e) {
      /* LOG.error("Can't update "+mapName+" map") */
      LOG.cant_update_map(mapName).error();
      throw e;
    } finally {
      if (br != null) {
        try {
          ...
          LOG.cant_close_bufferedreader_command_result(e1.toString()).error();
        }
      }
    }
  }
  ...
  synchronized public void updateMaps() throws IOException {
    ...
    if (!OS.startsWith("Linux") && !OS.startsWith("Mac")) {
      ...
      LOG.platform_not_supported_cant_update_user_(OS).error();
      return;
    }
  ...
  synchronized public String getUserName(int uid, String unknown) {
    ...
    if (uname == null) {
      ...
      LOG.cant_find_user_name_for_uid_use(uid, unknown).warn();
      uname = unknown;
    }
    return uname;
  }
  ...
  synchronized public String getGroupName(int gid, String unknown) {
    ...
    if (gname == null) {
      ...
      LOG.cant_find_group_name_for_gid_use(gid, unknown).warn();
      gname = unknown;
    }
    return gname;
  }
  ...
  public int getUidAllowingUnknown(String user) {
    ...
    try {
      ...
      LOG.cant_map_user_use_its_string(user, uid, e.toString()).info();
    }
    return uid;
  }
  ...
  public int getGidAllowingUnknown(String group) {
    ...
    try {
      ...
      LOG.cant_map_group_use_its_string(group, gid, e.toString()).info();
    }
    return gid;
  }
}
