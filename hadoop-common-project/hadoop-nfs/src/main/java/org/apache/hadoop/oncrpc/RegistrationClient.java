...
public class RegistrationClient extends SimpleTcpClient {
  public static final /* LogLOG=LogFactory.getLog(RegistrationClient.class); */
  ...
  static class RegistrationClientHandler extends SimpleTcpClientHandler {
    ...
    private boolean validMessageLength(int len) {
      ...
      if (len < 28) {
        if (LogGlobal.isDebugEnabled()) {
          ...
          LOG.portmap_mapping_registration_failed_resp(len).debug();
        }
        return false;
      }
      return true;
    ...
    private void handle(RpcDeniedReply deniedReply) {
      ...
      LOG.portmap_mapping_registration_request_was(deniedReply.toString()).warn();
    }
    ...
    private void handle(RpcAcceptedReply acceptedReply, XDR xdr) {
      ...
      if (answer != true) {
        ...
        LOG.portmap_mapping_registration_failed_acce(acceptState.toString()).warn();
      }
      /* LOG.info("Portmap mapping registration succeeded") */
      LOG.portmap_mapping_registration_succeeded().info();
    }
  }
}
