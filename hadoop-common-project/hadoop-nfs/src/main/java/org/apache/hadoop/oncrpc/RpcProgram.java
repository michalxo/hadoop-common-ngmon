...
public abstract class RpcProgram extends SimpleChannelUpstreamHandler {
  private static final /* LogLOG=LogFactory.getLog(RpcProgram.class); */
  ...
  public void register(int transport, int boundPort) {
    if (boundPort != port) {
      ...
      LOG.bound_port_different_with_configured_por(boundPort, port).info();
      port = boundPort;
    }
  ...
  protected void register(PortmapMapping mapEntry, boolean set) {
    ...
    try {
      ...
      LOG.failure_with_portmap_entry(request, host, port, mapEntry.toString()).error();
  ...
  public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
    ...
    if (LogGlobal.isTraceEnabled()) {
      ...
      LOG.procedure(program, String.valueOf(call.getProcedure())).tag("methodCall").trace();
    }
    
    if (this.progNumber != call.getProgram()) {
      /* LOG.warn("Invalid RPC call program "+call.getProgram()) */
      LOG.invalid_rpc_call_program(String.valueOf(call.getProgram())).tag("methodCall").warn();
    ...
    if (ver < lowProgVersion || ver > highProgVersion) {
      ...
      LOG.invalid_rpc_call_version(ver).warn();
      RpcAcceptedReply reply = RpcAcceptedReply.getInstance(call.getXid(),
