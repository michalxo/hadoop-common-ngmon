...
public class NfsExports {
  ...
  public static final /* LogLOG=LogFactory.getLog(NfsExports.class); */
  ...
  NfsExports(int cacheSize, long expirationPeriodNano, String matchHosts) {
    ...
    for(String mStr : matchStrings) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.processing_match_string(mStr).debug();
      }
      mStr = mStr.trim();
      if(!mStr.isEmpty()) {
  ...
  private static class CIDRMatch extends Match {
    ...
    public boolean isIncluded(String address, String hostname) {
      if(subnetInfo.isInRange(address)) {
        if(LogGlobal.isDebugEnabled()) {
          /* LOG.debug("CIDRNMatcher low = "+subnetInfo.getLowAddress()+", high = "+subnetInfo.getHighAddress()+", allowing client '"+address+"', '"+hostname+"'") */
          LOG.cidrnmatcher_low_high_allowing_client(String.valueOf(subnetInfo.getLowAddress()), String.valueOf(subnetInfo.getHighAddress()), address, hostname).tag("methodCall").debug();
        }
        return true;
      }
      if(LogGlobal.isDebugEnabled()) {
        /* LOG.debug("CIDRNMatcher low = "+subnetInfo.getLowAddress()+", high = "+subnetInfo.getHighAddress()+", denying client '"+address+"', '"+hostname+"'") */
        LOG.cidrnmatcher_low_high_denying_client(String.valueOf(subnetInfo.getLowAddress()), String.valueOf(subnetInfo.getHighAddress()), address, hostname).tag("methodCall").debug();
      }
      return false;
    }
    ...
    public boolean isIncluded(String address, String hostname) {
      if(ipOrHost.equalsIgnoreCase(address) ||
        ...
        if(LogGlobal.isDebugEnabled()) {
          ...
          LOG.exactmatcher_allowing_client(ipOrHost, address, hostname).debug();
        }
        return true;
      }
      if(LogGlobal.isDebugEnabled()) {
        /* LOG.debug("ExactMatcher '"+ipOrHost+"', denying client "+"'"+address+"', '"+hostname+"'") */
        LOG.exactmatcher_denying_client(ipOrHost, address, hostname).debug();
      }
      return false;
    }
    ...
    public boolean isIncluded(String address, String hostname) {
      if (pattern.matcher(address).matches()
        ...
        if (LogGlobal.isDebugEnabled()) {
          ...
          LOG.regexmatcher_allowing_client(String.valueOf(pattern.pattern()), address, hostname).tag("methodCall").debug();
        }
        return true;
      }
      if (LogGlobal.isDebugEnabled()) {
        /* LOG.debug("RegexMatcher '"+pattern.pattern()+"', denying client '"+address+"', '"+hostname+"'") */
        LOG.regexmatcher_denying_client(String.valueOf(pattern.pattern()), address, hostname).tag("methodCall").debug();
      }
      return false;
    }
  ...
  private static Match getMatch(String line) {
    ...
    if (host.equals("*")) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.using_match_all_for_and(host, privilege.toString()).debug();
      }
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.using_cidr_match_for_and(host, privilege.toString()).debug();
      }
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.using_regex_match_for_and(host, privilege.toString()).debug();
      }
      return new RegexMatch(privilege, host);
    }
    if (LogGlobal.isDebugEnabled()) {
      /* LOG.debug("Using exact match for '"+host+"' and "+privilege) */
      LOG.using_exact_match_for_and(host, privilege.toString()).debug();
    }
