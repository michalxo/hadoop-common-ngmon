...
import log_events.org.apache.hadoop.PortmapNamespace;
...
final class RpcProgramPortmap extends IdleStateAwareChannelUpstreamHandler {
  ...
  private static final /* LogLOG=LogFactory.getLog(RpcProgramPortmap.class); */
			PortmapNamespace LOG = LoggerFactory.getLogger(PortmapNamespace.class, new SimpleLogger());
  ...
  private XDR unset(int xid, XDR in, XDR out) {
    ...
    if (LogGlobal.isDebugEnabled())
      ...
      LOG.portmap_remove_key(key).debug();
  ...
  private XDR getport(int xid, XDR in, XDR out) {
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.portmap_getport_key(key, mapping.toString()).debug();
    }
    ...
    if (value != null) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.found_mapping_for_key_port(key, res).debug();
      }
    } else {
      /* LOG.warn("Warning, no mapping for key: "+key) */
      LOG.warning_mapping_for_key(key).warn();
    }
  ...
  public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
    ...
    if (portmapProc == PMAPPROC_NULL) {
      ...
      LOG.portmaphandler_unknown_rpc_procedure(portmapProc).info();
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) {
    ...
    LOG.encountered(String.valueOf(e.getCause())).tag("methodCall").warn();
    e.getChannel().close();
  }
}
