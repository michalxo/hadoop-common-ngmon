...
public class KerberosAuthenticator implements Authenticator {
  ...
  private static /* LoggerLOG=LoggerFactory.getLogger(KerberosAuthenticator.class); */
  ...
  public void authenticate(URL url, AuthenticatedURL.Token token)
    ...
    if (!token.isSet()) {
      ...
      if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
        ...
        LOG.jdk_performed_authentication_our_behalf().debug();
        ...
        LOG.performing_our_own_spnego_sequence().debug();
        doSpnegoSequence(token);
      } else {
        /* LOG.debug("Using fallback authenticator sequence.") */
        LOG.using_fallback_authenticator_sequence().debug();
  ...
  private void doSpnegoSequence(AuthenticatedURL.Token token) throws IOException, AuthenticationException {
    try {
      ...
      if (subject == null) {
        ...
        LOG.subject_context_logging().debug();
        subject = new Subject();
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.using_subject(subject.toString()).debug();
      }
