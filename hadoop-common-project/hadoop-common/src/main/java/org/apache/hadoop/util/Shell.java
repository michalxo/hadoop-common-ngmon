...
abstract public class Shell {
  ...
  public static final /* LogLOG=LogFactory.getLog(Shell.class); */
  ...
  private static String checkHadoopHome() {
    ...
    try {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.failed_detect_valid_hadoop_home_director(ioe.toString()).debug();
      }
      home = null;
    }
  ...
  public static final String getWinUtilsPath() {
    ...
    try {
      if (WINDOWS) {
       ...
       LOG.failed_locate_winutils_binary_hadoop_bin(ioe.toString()).error();
    }
  ...
  private static boolean isSetsidSupported() {
    ...
    try {
      ...
      LOG.setsid_not_available_this_machine_not().debug();
      setsidSupported = false;
    } finally { // handle the exit code
      if (LogGlobal.isDebugEnabled()) {
        /* LOG.debug("setsid exited with exit code "+(shexec!=null?shexec.getExitCode():"(null executor)")) */
        LOG.setsid_exited_with_exit_code(String.valueOf((shexec!=null?shexec.getExitCode():"(null executor)"))).tag("ternary-operator").debug();
      }
    }
  ...
  private void runCommand() throws IOException { 
    ...
    Thread errThread = new Thread() {
      ...
      public void run() {
        try {
          ...
          LOG.error_reading_error_stream(ioe.toString()).warn();
        }
      }
    };
    try {
    ...
    try {
      ...
      try {
        ...
        LOG.interrupted_while_reading_error_stream(ie.toString()).warn();
      }
      completed.set(true);
      ...
      try {
        ...
        LOG.error_while_closing_input_stream(ioe.toString()).warn();
      }
      try {
        if (!completed.get()) {
          errThread.interrupt();
        ...
        LOG.interrupted_while_joining_errthread().warn();
      }
      try {
        ...
        LOG.error_while_closing_error_stream(ioe.toString()).warn();
      }
      process.destroy();
      lastTime = Time.now();
    }
