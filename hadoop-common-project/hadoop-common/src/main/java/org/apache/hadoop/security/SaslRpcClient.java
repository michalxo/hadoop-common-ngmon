...
public class SaslRpcClient {
  public static final /* LogLOG=LogFactory.getLog(SaslRpcClient.class); */
  ...
  private SaslAuth selectSaslClient(List<SaslAuth> authTypes)
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.use_authentication_for_protocol(String.valueOf(selectedAuthType.getMethod()), String.valueOf(protocol.getSimpleName())).tag("methodCall").debug();
    }
  ...
  private SaslClient createSaslClient(SaslAuth authType)
    ...
    switch (method) {
      ...
      case KERBEROS: {
        ...
        if (LogGlobal.isDebugEnabled()) {
          ...
          LOG.rpc_servers_kerberos_principal_name_for(String.valueOf(protocol.getCanonicalName()), serverPrincipal).tag("methodCall").debug();
        }
        break;
      }
      default:
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.creating_sasl_client_authenticate_servic(mechanism, method.toString(), saslServerName).debug();
    }
  ...
  private Token<?> getServerToken(SaslAuth authType) throws IOException {
    ...
    LOG.get_token_info_proto_info(protocol.toString(), tokenInfo.toString()).debug();
  ...
  String getServerPrincipal(SaslAuth authType) throws IOException {
    ...
    LOG.get_kerberos_info_proto_info(protocol.toString(), krbInfo.toString()).debug();
    ...
    if (serverKeyPattern != null && !serverKeyPattern.isEmpty()) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.getting_serverkey_conf_value_principal(serverKey, String.valueOf(conf.get(serverKey)), confPrincipal).tag("methodCall").debug();
      }
  ...
  public AuthMethod saslConnect(InputStream inS, OutputStream outS)
    ...
    do {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.received_sasl_message(saslMessage.toString()).debug();
      }
  ...
  class WrappedInputStream extends FilterInputStream {
    ...
    private void readNextRpcPacket() throws IOException {
      ...
      LOG.reading_next_wrapped_rpc_packet().debug();
      ...
      if (headerBuilder.getCallId() == AuthProtocol.SASL.callId) {
        ...
        if (saslMessage.getState() == SaslState.WRAP) {
          ...
          if (LogGlobal.isDebugEnabled()) {
            ...
            LOG.unwrapping_token_length(String.valueOf(token.length)).tag("methodCall").debug();
          }
  ...
  class WrappedOutputStream extends FilterOutputStream {
    ...
    public void write(byte[] buf, int off, int len) throws IOException {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.wrapping_token_length(len).debug();
      }
    ...
    public void handle(Callback[] callbacks)
      ...
      if (nc != null) {
        if (LogGlobal.isDebugEnabled())
          ...
          LOG.sasl_client_callback_setting_username(userName).debug();
        nc.setName(userName);
      }
      if (pc != null) {
        if (LogGlobal.isDebugEnabled())
          /* LOG.debug("SASL client callback: setting userPassword") */
          LOG.sasl_client_callback_setting_userpasswor().debug();
        pc.setPassword(userPassword);
      }
      if (rc != null) {
        if (LogGlobal.isDebugEnabled())
          /* LOG.debug("SASL client callback: setting realm: "+rc.getDefaultText()) */
          LOG.sasl_client_callback_setting_realm(String.valueOf(rc.getDefaultText())).tag("methodCall").debug();
