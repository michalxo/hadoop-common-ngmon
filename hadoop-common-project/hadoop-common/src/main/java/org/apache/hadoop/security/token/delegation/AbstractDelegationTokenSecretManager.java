...
extends AbstractDelegationTokenIdentifier> 
  ...
  private static final /* LogLOG=LogFactory.getLog(AbstractDelegationTokenSecretManager.class); */
  ...
  public synchronized void addPersistedDelegationToken(
    ...
    if (dKey == null) {
      ...
      LOG.key_found_for_persisted_identifier(String.valueOf(identifier.toString())).tag("methodCall").warn();
      return;
    }
  ...
  private void updateCurrentKey() throws IOException {
    ...
    LOG.updating_current_master_key_for_generati().info();
  ...
  protected synchronized byte[] createPassword(TokenIdent identifier) {
    ...
    LOG.creating_password_for_identifier(identifier.toString()).info();
  ...
  public synchronized long renewToken(Token<TokenIdent> token,
    ...
    LOG.token_renewal_requested_for_identifier(id.toString()).info();
  ...
  public synchronized TokenIdent cancelToken(Token<TokenIdent> token,
    ...
    LOG.token_cancelation_requested_for_identifi(id.toString()).info();
  ...
  public void stopThreads() {
    if (LogGlobal.isDebugEnabled())
      ...
      LOG.stopping_expired_delegation_token_remove().debug();
    running = false;
  ...
  private class ExpiredTokenRemover extends Thread {
    ...
    public void run() {
      ...
      LOG.starting_expired_delegation_token_remove(tokenRemoverScanInterval/(60*1000)).tag("mathExp").info();
      try {
        while (running) {
          long now = Time.now();
          if (lastMasterKeyUpdate + keyUpdateInterval < now) {
            try {
              ...
              LOG.master_key_updating_failed(e.toString()).error();
            }
          }
          ...
          try {
            ...
            LOG.interruptedexcpetion_recieved_for_expire(ie.toString()).error();
          }
        }
      } catch (Throwable t) {
        /* LOG.error("ExpiredTokenRemover thread received unexpected exception. "+t) */
        LOG.expiredtokenremover_thread_received_unex(t.toString()).error();
