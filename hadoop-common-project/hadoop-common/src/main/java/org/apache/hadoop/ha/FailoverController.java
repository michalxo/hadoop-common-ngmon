...
public class FailoverController {
  ...
  private static final /* LogLOG=LogFactory.getLog(FailoverController.class); */
  ...
  private void preFailoverChecks(HAServiceTarget from,
    ...
    try {
      ...
      LOG.msgegetlocalized(msg, String.valueOf(e.getLocalizedMessage())).tag("methodCall").error();
    ...
    if (!toSvcStatus.isReadyToBecomeActive()) {
      ...
      if (!forceActive) {
        ...
        LOG.service_not_ready_become_active_but(notReadyReason).warn();
      }
    }
  ...
  boolean tryGracefulFence(HAServiceTarget svc) {
    ...
    try {
      ...
      LOG.unable_gracefully_make_standby(svc.toString(), String.valueOf(sfe.getMessage())).tag("methodCall").warn();
    } catch (IOException ioe) {
      /* LOG.warn("Unable to gracefully make "+svc+" standby (unable to connect)",ioe) */
      LOG.unable_gracefully_make_standby_unable_co(svc.toString(), ioe.toString()).warn();
    } finally {
      if (proxy != null) {
        RPC.stopProxy(proxy);
      }
  ...
  public void failover(HAServiceTarget fromSvc,
    ...
    try {
      ...
      LOG.unable_make_active_failing_back(toSvc.toString(), String.valueOf(sfe.getMessage())).tag("methodCall").error();
      failed = true;
      cause = sfe;
    } catch (IOException ioe) {
      /* LOG.error("Unable to make "+toSvc+" active (unable to connect). Failing back.",ioe) */
      LOG.unable_make_active_unable_connect_failin(toSvc.toString(), ioe.toString()).error();
      failed = true;
      cause = ioe;
    }
