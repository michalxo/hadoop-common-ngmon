...
public abstract class FileSystem extends Configured implements Closeable {
  ...
  public static final /* LogLOG=LogFactory.getLog(FileSystem.class); */
    ...
    synchronized (deleteOnExit) {
      for (Iterator<Path> iter = deleteOnExit.iterator(); iter.hasNext();) {
        ...
        try {
          ...
          LOG.ignoring_failure_deleteonexit_for_path(path.toString()).info();
        }
        iter.remove();
      }
    }
    ...
    private class ClientFinalizer implements Runnable {
      ...
      public synchronized void run() {
        try {
          ...
          LOG.filesystem_cache_closeall_threw_exceptio(e.toString()).info();
        }
      }
    }
