...
public class JniBasedUnixGroupsNetgroupMappingWithFallback implements
  ...
  private static final /* LogLOG=LogFactory.getLog(JniBasedUnixGroupsNetgroupMappingWithFallback.class); */
  ...
  public JniBasedUnixGroupsNetgroupMappingWithFallback() {
    if (NativeCodeLoader.isNativeCodeLoaded()) {
      ...
      LOG.falling_back_shell_based().info();
      this.impl = new ShellBasedUnixGroupsNetgroupMapping();
    }
    if (LogGlobal.isDebugEnabled()) {
      /* LOG.debug("Group mapping impl="+impl.getClass().getName()) */
      LOG.group_mapping_impl(String.valueOf(impl.getClass().getName())).tag("methodCall").debug();
    }
  }
