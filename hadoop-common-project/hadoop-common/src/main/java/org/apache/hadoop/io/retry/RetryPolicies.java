...
public class RetryPolicies {
  ...
  public static final /* LogLOG=LogFactory.getLog(RetryPolicies.class); */
  ...
  public static class MultipleLinearRandomRetry implements RetryPolicy {
    ...
    public static MultipleLinearRandomRetry parseCommaSeparatedString(String s) {
      ...
      if (elements.length == 0) {
        ...
        LOG.illegal_value_there_element(s).warn();
        return null;
      }
      if (elements.length % 2 != 0) {
        /* LOG.warn("Illegal value: the number of elements in \""+s+"\" is "+elements.length+" but an even number of elements is expected.") */
        LOG.illegal_value_number_elements_but_even(s, String.valueOf(elements.length)).tag("methodCall").warn();
        return null;
      }
    ...
    private static int parsePositiveInt(final String[] elements,
      ...
      try {
        ...
        LOG.failed_parse_which_index_element(s, i, originalString, nfe.toString()).warn();
        return -1;
      }

      if (n <= 0) {
        /* LOG.warn("The value "+n+" <= 0: it is parsed from the string \""+s+"\" which is the index "+i+" element in \""+originalString+"\"") */
        LOG.value_parsed_from_string_which_index_ele(n, s, i, originalString).warn();
        return -1;
      }
      return n;
    }
