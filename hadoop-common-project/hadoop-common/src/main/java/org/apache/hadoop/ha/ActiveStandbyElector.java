...
public class ActiveStandbyElector implements StatCallback, StringCallback {
  ...
  public synchronized void joinElection(byte[] data)
    ...
    if (wantToBeInElection) {
      ...
      LOG.already_election_not_connecting().info();
      return;
    }
    ...
    LOG.attempting_active_election_for(this.toString()).debug();
  ...
  public synchronized void ensureParentZNode()
    ...
    for (int i = 1; i < pathParts.length; i++) {
      ...
      LOG.ensuring_existence(prefixPath).debug();
      try {
    ...
    LOG.successfully_created(znodeWorkingDir).info();
  }
  ...
  public synchronized void clearParentZNode()
    ...
    try {
      ...
      LOG.recursively_deleting_from(znodeWorkingDir).info();
    ...
    LOG.successfully_deleted_from(znodeWorkingDir).info();
  }
  ...
  public synchronized void quitElection(boolean needFence) {
    ...
    LOG.yielding_from_election().info();
  ...
  public synchronized void processResult(int rc, String path, Object ctx,
    ...
    LOG.createnode_result_for_path_connectionsta(rc, path, zkConnectionState.toString(), this.toString()).debug();
    ...
    LOG.errormessage(errorMessage).debug();

    if (shouldRetry(code)) {
      if (createRetryCount < NUM_RETRIES) {
        /* LOG.debug("Retrying createNode createRetryCount: "+createRetryCount) */
        LOG.retrying_createnode_createretrycount(createRetryCount).debug();
        ++createRetryCount;
        createLockNodeAsync();
        return;
      }
      ...
      LOG.lock_acquisition_failed_because_session_().warn();
      return;
    }
  ...
  public synchronized void processResult(int rc, String path, Object ctx,
    ...
    LOG.statnode_result_for_path_connectionstate(rc, path, zkConnectionState.toString(), this.toString()).debug();
    ...
    if (shouldRetry(code)) {
      ...
      LOG.lock_monitoring_failed_because_session_w().warn();
      return;
    }
  ...
  synchronized void processWatchEvent(ZooKeeper zk, WatchedEvent event) {
    ...
    LOG.watcher_event_type_with_state_for_connec(eventType.toString(), String.valueOf(event.getState()), String.valueOf(event.getPath()), zkConnectionState.toString(), this.toString()).tag("methodCall").debug();
    ...
    if (eventType == Event.EventType.None) {
      ...
      case Disconnected:
        ...
        LOG.session_disconnected_entering_neutral_mo().info();
      ...
      case Expired:
        ...
        LOG.session_expired_entering_neutral_mode_an().info();
        enterNeutralMode();
        reJoinElection(0);
        break;
      case SaslAuthenticated:
        /* LOG.info("Successfully authenticated to ZooKeeper using SASL.") */
        LOG.successfully_authenticated_zookeeper_usi().info();
        break;
      default:
    ...
    if (path != null) {
      ...
      default:
        ...
        LOG.unexpected_node_event_for_path(eventType.toString(), path).debug();
        monitorActiveStatus();
      }
  ...
  private void fatalError(String errorMessage) {
    ...
    LOG.errormessage(errorMessage).fatal();
    reset();
  ...
  private void monitorActiveStatus() {
    ...
    LOG.monitoring_active_leader_for(this.toString()).debug();
    statRetryCount = 0;
    monitorLockNodeAsync();
  }
  ...
  private void reJoinElection(int sleepTime) {
    ...
    LOG.trying_establish_session().info();
    ...
    try {
      ...
      if (appData != null) {
        ...
        LOG.not_joining_election_since_service_has_r().info();
      }
    } finally {
  ...
  private boolean reEstablishSession() {
    ...
    while(!success && connectionRetryCount < NUM_RETRIES) {
      ...
      LOG.establishing_zookeeper_connection_for(this.toString()).debug();
      try {
        createConnection();
        success = true;
      } catch(IOException e) {
        /* LOG.warn(e) */
        LOG.exception(e.toString()).warn();
        sleepFor(5000);
      } catch(KeeperException e) {
        /* LOG.warn(e) */
        LOG.exception(e.toString()).warn();
  ...
  private void createConnection() throws IOException, KeeperException {
    ...
    LOG.created_new_connection_for(this.toString()).debug();
  }
  ...
  public synchronized void terminateConnection() {
    ...
    LOG.terminating_connection_for(this.toString()).debug();
  ...
  private boolean becomeActive() {
    ...
    try {
      ...
      LOG.becoming_active_for(this.toString()).debug();
      appClient.becomeActive();
      state = State.ACTIVE;
      return true;
    } catch (Exception e) {
      /* LOG.warn("Exception handling the winning of election",e) */
      LOG.exception_handling_winning_election(e.toString()).warn();
  ...
  private void writeBreadCrumbNode(Stat oldBreadcrumbStat)
    ...
    LOG.writing_znode_indicate_that_local_node(zkBreadCrumbPath).info();
  ...
  private void tryDeleteOwnBreadCrumbNode() {
    ...
    LOG.deleting_bread_crumb_active_node().info();
    ...
    try {
      ...
      LOG.unable_delete_our_own_bread_crumb_expect(zkBreadCrumbPath, String.valueOf(e.getLocalizedMessage())).tag("methodCall").warn();
    }
  }
  ...
  private Stat fenceOldActive() throws InterruptedException, KeeperException {
    ...
    LOG.checking_for_any_old_active_which().info();
    try {
    ...
    LOG.old_node_exists(StringUtils.byteToHexString(data)).tag("methodCall").info();
    if (Arrays.equals(data, appData)) {
      /* LOG.info("But old node has our own data, so don't need to fence it.") */
      LOG.but_old_node_has_our_own().info();
    } else {
  ...
  private void becomeStandby() {
    if (state != State.STANDBY) {
      ...
      LOG.becoming_standby_for(this.toString()).debug();
      state = State.STANDBY;
  ...
  private void enterNeutralMode() {
    if (state != State.NEUTRAL) {
      ...
      LOG.entering_neutral_mode_for(this.toString()).debug();
      state = State.NEUTRAL;
  ...
  private synchronized boolean isStaleClient(Object ctx) {
    ...
    if (zkClient != (ZooKeeper)ctx) {
      /* LOG.warn("Ignoring stale result from old client with sessionId "+String.format("0x%08x",((ZooKeeper)ctx).getSessionId())) */
      LOG.ignoring_stale_result_from_old_client(String.valueOf(String.format("0x%08x",((ZooKeeper)ctx).getSessionId()))).warn();
      return true;
    }
    return false;
  }
    ...
    private void waitForZKConnectionEvent(int connectionTimeoutMs)
      ...
      try {
        if (!hasReceivedEvent.await(connectionTimeoutMs, TimeUnit.MILLISECONDS)) {
          ...
          LOG.connection_timed_out_couldnt_connect_zoo(connectionTimeoutMs).error();
          zk.close();
