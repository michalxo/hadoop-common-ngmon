...
public class ClientCache {
  ...
  public synchronized Client getClient(Configuration conf,
    ...
    if (LogGlobal.isDebugEnabled()) {
      Client.LOG.getting_client_out_of_cache(client.toString()).debug();
    }
    return client;
  }
  ...
  public void stopClient(Client client) {
    if (LogGlobal.isDebugEnabled()) {
      Client.LOG.stopping_client_from_cache(client.toString()).debug();
    }
    synchronized (this) {
      client.decCount();
      if (client.isZeroReference()) {
        if (LogGlobal.isDebugEnabled()) {
          Client.LOG.removing_client_from_cache(client.toString()).debug();
        }
    ...
    if (client.isZeroReference()) {
      if (LogGlobal.isDebugEnabled()) {
        Client.LOG.stopping_client_no_more_references(client.toString()).debug();
      }
      client.stop();
    }
  }
