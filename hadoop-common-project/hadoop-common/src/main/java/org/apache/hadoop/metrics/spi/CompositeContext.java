...
import log_events.org.apache.hadoop.MetricsNamespace;
...
public class CompositeContext extends AbstractMetricsContext {
  ...
  private static final /* LogLOG=LogFactory.getLog(CompositeContext.class); */
			MetricsNamespace LOG = LoggerFactory.getLogger(MetricsNamespace.class, new SimpleLogger());
  ...
  public void init(String contextName, ContextFactory factory) {
    ...
    try {
      ...
      /* LOG.error("Unable to initialize composite metric "+contextName+": could not init arity",e) */
      LOG.unable_initialize_composite_metric_could(contextName, e.toString()).error();
      return;
    }
  ...
  protected void emitRecord(String contextName, String recordName,
    ...
    for (MetricsContext ctxt : subctxt) {
      try {
        ...
        LOG.emitrecord_failed(String.valueOf(ctxt.getContextName()), e.toString()).tag("methodCall").warn();
      }
    }
  }
  ...
  protected void flush() throws IOException {
    for (MetricsContext ctxt : subctxt) {
      try {
        ...
        LOG.flush_failed(String.valueOf(ctxt.getContextName()), e.toString()).tag("methodCall").warn();
      }
    }
  }
  ...
  public void startMonitoring() throws IOException {
    for (MetricsContext ctxt : subctxt) {
      try {
        ...
        LOG.startmonitoring_failed(String.valueOf(ctxt.getContextName()), e.toString()).tag("methodCall").warn();
      }
    }
  }
