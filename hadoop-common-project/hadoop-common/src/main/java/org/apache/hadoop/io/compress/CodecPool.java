...
public class CodecPool {
  private static final /* LogLOG=LogFactory.getLog(CodecPool.class); */
  ...
  public static Compressor getCompressor(CompressionCodec codec, Configuration conf) {
    ...
    if (compressor == null) {
      ...
      LOG.got_brand_new_compressor(String.valueOf(codec.getDefaultExtension())).tag("methodCall").info();
    } else {
      compressor.reinit(conf);
      if(LogGlobal.isDebugEnabled()) {
        /* LOG.debug("Got recycled compressor") */
        LOG.got_recycled_compressor().debug();
      }
    }
  ...
  public static Decompressor getDecompressor(CompressionCodec codec) {
    ...
    if (decompressor == null) {
      ...
      LOG.got_brand_new_decompressor(String.valueOf(codec.getDefaultExtension())).tag("methodCall").info();
    } else {
      if(LogGlobal.isDebugEnabled()) {
        /* LOG.debug("Got recycled decompressor") */
        LOG.got_recycled_decompressor().debug();
      }
    }
