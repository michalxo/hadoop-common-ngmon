...
public abstract class AbstractGangliaSink implements MetricsSink {
  ...
  public void init(SubsetConfiguration conf) {
    ...
    LOG.initializing_gangliasink_for_ganglia_met().debug();
    ...
    try {
      ...
      /* LOG.error(se) */
      LOG.exception(se.toString()).error();
    }
    ...
    if (propertyarr != null && propertyarr.length > 0) {
      for (String metricNValue : propertyarr) {
        ...
        if (metricNValueArr.length != 2 || metricNValueArr[0].length() == 0) {
          ...
          LOG.invalid_propertylist_for(String.valueOf(gtype.name())).tag("methodCall").error();
        }
