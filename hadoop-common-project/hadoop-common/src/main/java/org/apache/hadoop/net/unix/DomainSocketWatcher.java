...
public final class DomainSocketWatcher implements Closeable {
  ...
  static /* LogLOG=LogFactory.getLog(DomainSocketWatcher.class); */
    ...
    public boolean handle(DomainSocket sock) {
      try {
        if (LogGlobal.isTraceEnabled()) {
          ...
          LOG.notificationhandler_doing_read(this.toString(), String.valueOf(sock.fd)).tag("methodCall").trace();
        }
        if (sock.getInputStream().read() == -1) {
          if (LogGlobal.isTraceEnabled()) {
            /* LOG.trace(this+": NotificationHandler: got EOF on "+sock.fd) */
            LOG.notificationhandler_got_eof(this.toString(), String.valueOf(sock.fd)).tag("methodCall").trace();
          }
          throw new EOFException();
        }
        if (LogGlobal.isTraceEnabled()) {
          /* LOG.trace(this+": NotificationHandler: read succeeded on "+sock.fd) */
          LOG.notificationhandler_read_succeeded(this.toString(), String.valueOf(sock.fd)).tag("methodCall").trace();
        }
        return false;
      } catch (IOException e) {
        if (LogGlobal.isTraceEnabled()) {
          /* LOG.trace(this+": NotificationHandler: setting closed to "+"true for "+sock.fd) */
          LOG.notificationhandler_setting_closed_true_(this.toString(), String.valueOf(sock.fd)).tag("methodCall").trace();
        }
        closed = true;
        return true;
      }
  ...
  public void close() throws IOException {
    ...
    try {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.closing(this.toString()).debug();
      }
      closed = true;
    } finally {
      lock.unlock();
  ...
  private void kick() {
    try {
      ...
      if (!closed) {
        ...
        LOG.error_writing_notificationsockets(this.toString(), e.toString()).error();
      }
    }
  }
  ...
  private void sendCallback(String caller, TreeMap<Integer, Entry> entries,
    ...
    if (LogGlobal.isTraceEnabled()) {
      ...
      LOG.starting_sendcallback_for(this.toString(), caller, fd).trace();
    }
    ...
    if (entry.getHandler().handle(sock)) {
      if (LogGlobal.isTraceEnabled()) {
        ...
        LOG.closing_request_handler(this.toString(), caller, fd).trace();
      }
      if (toRemove.remove(fd) != null) {
        if (LogGlobal.isTraceEnabled()) {
          /* LOG.trace(this+": "+caller+" : sendCallback processed fd "+fd+" in toRemove.") */
          LOG.sendcallback_processed_toremove(this.toString(), caller, fd).trace();
        }
      }
      try {
      ...
      if (LogGlobal.isTraceEnabled()) {
        ...
        LOG.sendcallback_not_closing(this.toString(), caller, fd).trace();
      }
    }
  }
  ...
  final Thread watcherThread = new Thread(new Runnable() {
    ...
    public void run() {
      if (LogGlobal.isDebugEnabled()) {
        /* LOG.debug(this+": starting with interruptCheckPeriodMs = "+interruptCheckPeriodMs) */
        LOG.starting_with_interruptcheckperiodms(this.toString(), interruptCheckPeriodMs).debug();
      }
      ...
      try {
          ...
          try {
            ...
            if (!(toAdd.isEmpty() && toRemove.isEmpty())) {
              ...
              for (Iterator<Entry> iter = toAdd.iterator(); iter.hasNext(); ) {
                ...
                if (LogGlobal.isTraceEnabled()) {
                  ...
                  LOG.adding(this.toString(), String.valueOf(sock.fd)).tag("methodCall").trace();
                }
                fdSet.add(sock.fd);
                iter.remove();
              }
            ...
            if (closed) {
              if (LogGlobal.isDebugEnabled()) {
                ...
                LOG.thread_terminating(toString()).tag("methodCall").debug();
              }
              return;
            }
        ...
        LOG.terminating_interruptedexception(toString()).tag("methodCall").info();
      } catch (IOException e) {
        /* LOG.error(toString()+" terminating on IOException",e) */
        LOG.terminating_ioexception(toString(), e.toString()).tag("methodCall").error();
      } finally {
  ...
  private void addNotificationSocket(final TreeMap<Integer, Entry> entries,
    ...
    if (LogGlobal.isTraceEnabled()) {
      /* LOG.trace(this+": adding notificationSocket "+notificationSockets[1].fd+", connected to "+notificationSockets[0].fd) */
      LOG.adding_notificationsocket_connected(this.toString(), String.valueOf(notificationSockets[1].fd), String.valueOf(notificationSockets[0].fd)).trace();
    }
  }
