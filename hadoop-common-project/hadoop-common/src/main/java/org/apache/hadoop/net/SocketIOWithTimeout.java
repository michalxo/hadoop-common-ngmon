...
abstract class SocketIOWithTimeout {
  ...
  static final /* LogLOG=LogFactory.getLog(SocketIOWithTimeout.class); */
      ...
      void close() {
        if (selector != null) {
          try {
            ...
            LOG.unexpected_exception_while_closing_selec(e.toString()).warn();
          }
        }
      }    
    }
    ...
    int select(SelectableChannel channel, int ops, long timeout) 
      ...
      try {
        ...
        try {
          ...
          LOG.unexpected_exception_while_clearing_sele(e.toString()).info();
