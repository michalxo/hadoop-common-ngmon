...
public abstract class HAAdmin extends Configured implements Tool {
  ...
  private static final /* LogLOG=LogFactory.getLog(HAAdmin.class); */
  ...
  private boolean checkManualStateManagementOK(HAServiceTarget target) {
    if (target.isAutoFailoverEnabled()) {
      if (requestSource != RequestSource.REQUEST_BY_USER_FORCED) {
        ...
        LOG.proceeding_with_manual_state_management_(target.toString()).warn();
        return true;
      }
    }
    return true;
  ...
  public int run(String[] argv) throws Exception {
    try {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.operation_failed(ioe.toString()).debug();
      }
      return -1;
    }
  }
