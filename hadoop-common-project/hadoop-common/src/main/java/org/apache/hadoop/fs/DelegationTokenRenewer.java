...
public class DelegationTokenRenewer
    extends Thread {
  private static final /* LogLOG=LogFactory.getLog(DelegationTokenRenewer.class); */
  ...
  public <T extends FileSystem & Renewable> void removeRenewAction(
    ...
    if (queue.remove(action)) {
      try {
        ...
        LOG.interrupted_while_canceling_token_for_fi(String.valueOf(fs.getUri())).tag("methodCall").error();
        if (LogGlobal.isDebugEnabled()) {
          /* LOG.debug(ie.getStackTrace()) */
          LOG.iegetstacktrace(String.valueOf(ie.getStackTrace())).tag("methodCall").debug();
        }
      }
    }
  }
  ...
  public void run() {
    for(;;) {
      ...
      try {
        ...
        action.weakFs.get().LOG.failed_to_renew_token_action(action.toString(), ie.toString()).warn();
      }
    }
  }
}
