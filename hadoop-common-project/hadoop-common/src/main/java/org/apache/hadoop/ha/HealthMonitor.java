...
public class HealthMonitor {
  private static final /* LogLOG=LogFactory.getLog(HealthMonitor.class); */
  ...
  public void shutdown() {
    ...
    LOG.stopping_healthmonitor_thread().info();
    shouldRun = false;
    daemon.interrupt();
  }
  ...
  private void tryConnect() {
    ...
    try {
      ...
      LOG.could_not_connect_local_service(targetToMonitor.toString(), String.valueOf(e.getMessage())).tag("methodCall").warn();
      proxy = null;
  ...
  private void doHealthChecks() throws InterruptedException {
    while (shouldRun) {
      ...
      try {
        ...
        LOG.service_health_check_failed_for(targetToMonitor.toString(), String.valueOf(e.getMessage())).tag("methodCall").warn();
        enterState(State.SERVICE_UNHEALTHY);
      } catch (Throwable t) {
        /* LOG.warn("Transport-level exception trying to monitor health of "+targetToMonitor+": "+t.getLocalizedMessage()) */
        LOG.transport_level_exception_trying_monitor(targetToMonitor.toString(), String.valueOf(t.getLocalizedMessage())).tag("methodCall").warn();
        RPC.stopProxy(proxy);
        proxy = null;
        enterState(State.SERVICE_NOT_RESPONDING);
  ...
  private synchronized void enterState(State newState) {
    if (newState != state) {
      ...
      LOG.entering_state(newState.toString()).info();
      state = newState;
    ...
    private MonitorDaemon() {
      ...
      setUncaughtExceptionHandler(new UncaughtExceptionHandler() {
        ...
        public void uncaughtException(Thread t, Throwable e) {
          ...
          LOG.health_monitor_failed(e.toString()).fatal();
