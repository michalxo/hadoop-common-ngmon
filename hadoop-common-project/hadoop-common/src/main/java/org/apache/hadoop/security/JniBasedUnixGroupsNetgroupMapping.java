...
public class JniBasedUnixGroupsNetgroupMapping
  ...
  private static final /* LogLOG=LogFactory.getLog(JniBasedUnixGroupsNetgroupMapping.class); */
  ...
  static {
    ...
    LOG.using_jnibasedunixgroupsnetgroupmapping_().debug();
  }
  ...
  protected synchronized List<String> getUsersForNetgroup(String netgroup) {
    ...
    try {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.error_getting_users_for_netgroup(netgroup, e.toString()).debug();
      } else {
        /* LOG.info("Error getting users for netgroup "+netgroup+": "+e.getMessage()) */
        LOG.error_getting_users_for_netgroup(netgroup, String.valueOf(e.getMessage())).tag("methodCall").info();
      }
    }
