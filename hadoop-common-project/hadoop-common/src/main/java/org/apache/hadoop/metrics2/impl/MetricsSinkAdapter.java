...
class MetricsSinkAdapter implements SinkQueue.Consumer<MetricsBuffer> {
  ...
  private final /* LogLOG=LogFactory.getLog(MetricsSinkAdapter.class); */
  ...
  boolean putMetrics(MetricsBuffer buffer, long logicalTime) {
    if (logicalTime % period == 0) {
      ...
      LOG.enqueue_logicaltime(logicalTime).debug();
  ...
  public boolean putMetricsImmediate(MetricsBuffer buffer) {
    ...
    if (!queue.enqueue(waitableBuffer)) {
      ...
      LOG.has_full_queue_and_cant_consume(name).warn();
      dropped.incr();
      return false;
    }
    if (!waitableBuffer.waitTillNotified(oobPutTimeout)) {
      ...
      LOG.couldnt_fulfill_immediate_putmetrics_req(name).warn();
      return false;
    }
    return true;
  }
  ...
  void publishMetricsFromQueue() {
    ...
    while (!stopping) {
      try {
        ...
        LOG.thread_interrupted(name).info();
      } catch (Exception e) {
        if (n > 0) {
        ...
        for (MetricsRecordImpl record : entry.records()) {
          if ((context == null || context.equals(record.context())) &&
            ...
            if (LogGlobal.isDebugEnabled()) {
              ...
              LOG.pushing_record(String.valueOf(entry.name()), String.valueOf(record.context()), String.valueOf(record.name()), name).tag("methodCall").debug();
            }
  ...
  void stop() {
    ...
    try {
      ...
      LOG.stop_interrupted(e.toString()).warn();
    }
    if (sink instanceof Closeable) {
      IOUtils.cleanup(null, (Closeable)sink);
    }
  }
