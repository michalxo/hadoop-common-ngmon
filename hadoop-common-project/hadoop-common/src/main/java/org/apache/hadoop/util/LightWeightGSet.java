...
public class LightWeightGSet<K, E extends K> implements GSet<K, E> {
  ...
  public LightWeightGSet(final int recommended_length) {
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.recommended_actual(recommended_length, actual).debug();
    }
  ...
  static int computeCapacity(long maxMemory, double percentage,
    ...
    LOG.computing_capacity_for_map(mapName).info();
    /* LOG.info("VM type       = "+vmBit+"-bit") */
    LOG.type_bit(vmBit).info();
    ...
    LOG.max_memory(percentage, StringUtils.TraditionalBinaryPrefix.long2String(maxMemory,"B",1), StringUtils.TraditionalBinaryPrefix.long2String((long)percentMemory,"B",1)).tag("methodCall").info();
    /* LOG.info("capacity      = 2^"+exponent+" = "+c+" entries") */
    LOG.capacity_entries(exponent, c).info();
    return c;
  }
