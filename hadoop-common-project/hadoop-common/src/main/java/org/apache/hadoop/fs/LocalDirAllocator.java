...
public class LocalDirAllocator {
  ...
  private static class AllocatorPerContext {
      ...
      private final FsNamespace LOG = org.ngmon.logger.core.LoggerFactory.getLogger(FsNamespace.class, new SimpleLogger());
          ...
          try {
            ...
            if(localFS.mkdirs(tmpDir)|| localFS.exists(tmpDir)) {
              try {
                ...
                LOG.not_writablen(String.valueOf(localDirs[i]), de.toString()).warn();
              }
            } else {
              /* LOG.warn("Failed to create "+localDirs[i]) */
              LOG.failed_create(String.valueOf(localDirs[i])).warn();
            }
          } catch (IOException ie) { 
            /* LOG.warn("Failed to create "+localDirs[i]+": "+ie.getMessage()+"\n",ie) */
            LOG.failed_create(String.valueOf(localDirs[i]), String.valueOf(ie.getMessage()), ie.toString()).tag("methodCall").warn();
          } //ignore
        }
    ...
    private Path createPath(String path, 
      ...
      if (checkWrite) {
        ...
        try {
          ...
          LOG.disk_error_exception(d.toString()).warn();
          return null;
        }
      }
      return file;
