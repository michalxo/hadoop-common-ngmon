...
public class Bzip2Factory {
  private static final /* LogLOG=LogFactory.getLog(Bzip2Factory.class); */
  ...
  public static boolean isNativeBzip2Loaded(Configuration conf) {
    ...
    if (!bzip2LibraryName.equals(libname)) {
      ...
      if (libname.equals("java-builtin")) {
        ...
        LOG.using_pure_java_version_bzip_library().info();
        ...
        try {
          ...
          LOG.successfully_loaded_initialized_native_b(libname).info();
        } catch (Throwable t) {
          /* LOG.warn("Failed to load/initialize native-bzip2 library "+libname+", will use pure-Java version") */
          LOG.failed_load_initialize_native_bzip_libra(libname).warn();
        }
      }
    }
