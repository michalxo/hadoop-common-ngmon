...
public abstract class Server {
  ...
  public static void registerProtocolEngine(RPC.RpcKind rpcKind, 
    ...
    LOG.rpckind_rpcrequestwrapperclass_rpcinvoke(rpcKind.toString(), rpcRequestWrapperClass.toString(), rpcInvoker.toString()).debug();
  }
...
//  public static final /* LogLOG=LogFactory.getLog(Server.class); */
//			IpcNamespace LOG = LoggerFactory.getLogger(IpcNamespace.class, new SimpleLogger());
  public static final /* LogAUDITLOG=LogFactory.getLog("SecurityLogger."+Server.class.getName()); */
			IpcNamespace LOG = LoggerFactory.getLogger(IpcNamespace.class, new SimpleLogger());
  ...
  private class Listener extends Thread {
    ...
    private class Reader extends Thread {
      ...
      public void run() {
        ...
        LOG.starting(String.valueOf(Thread.currentThread().getName())).tag("methodCall").info();
        try {
          doRunLoop();
        } finally {
          try {
            ...
            LOG.error_closing_read_selector(String.valueOf(Thread.currentThread().getName()), ioe.toString()).tag("methodCall").error();
          }
        }
      }
        ...
        while (running) {
          ...
          try {
            ...
            if (running) {                      // unexpected -- log it
              ...
              LOG.unexpectedly_interrupted(String.valueOf(Thread.currentThread().getName()), e.toString()).tag("methodCall").info();
            }
          } catch (IOException ex) {
            /* LOG.error("Error in Reader",ex) */
            LOG.error_reader(ex.toString()).error();
          }
        }
      }
    ...
    public void run() {
      ...
      while (running) {
        ...
        try {
          ...
          LOG.out_memory_server_select(e.toString()).warn();
      ...
      LOG.stopping(String.valueOf(Thread.currentThread().getName())).tag("methodCall").info();
    ...
    void doRead(SelectionKey key) throws InterruptedException {
      ...
      try {
        ...
        LOG.readandprocess_caught_interruptedexcepti(String.valueOf(Thread.currentThread().getName()), ieo.toString()).tag("methodCall").info();
        throw ieo;
      } catch (Exception e) {
        ...
        /* LOG.info(Thread.currentThread().getName()+": readAndProcess from client "+c.getHostAddress()+" threw exception ["+e+"]",(einstanceofWrappedRpcServerException)?null:e) */
        LOG.readandprocess_from_client_threw_excepti(String.valueOf(Thread.currentThread().getName()), String.valueOf(c.getHostAddress()), e.toString()).tag("methodCall").info();
    ...
    synchronized void doStop() {
      ...
      if (acceptChannel != null) {
        try {
          ...
          LOG.exception_closing_listener_socket(String.valueOf(Thread.currentThread().getName()), e.toString()).tag("methodCall").info();
        }
      }
    ...
    public void run() {
      ...
      try {
        ...
        try {
          ...
          LOG.couldnt_close_write_selector(String.valueOf(Thread.currentThread().getName()), ioe.toString()).tag("methodCall").error();
        }
      }
    }
        ...
        try {
          ...
          while (iter.hasNext()) {
            ...
            try {
              ...
              LOG.doasyncwrite_threw_exception(String.valueOf(Thread.currentThread().getName()), e.toString()).tag("methodCall").info();
            }
          }
          long now = Time.now();
          ...
          if(LogGlobal.isDebugEnabled()) {
            ...
            LOG.checking_for_old_call_responses().debug();
          }
          ...
          LOG.exception_responder(e.toString()).warn();
        }
      }
    }
      ...
      synchronized(call.connection.responseQueue) {
        if (processResponse(call.connection.responseQueue, false)) {
          try {
            ...
            LOG.exception_while_changing_ops(e.toString()).warn();
          }
        }
      }
    }
      ...
      try {
        synchronized (responseQueue) {
          ...
          if (LogGlobal.isDebugEnabled()) {
            ...
            LOG.responding(String.valueOf(Thread.currentThread().getName()), call.toString()).tag("methodCall").debug();
          }
          //
          ...
          if (!call.rpcResponse.hasRemaining()) {
            ...
            if (LogGlobal.isDebugEnabled()) {
              ...
              LOG.responding_wrote_bytes(String.valueOf(Thread.currentThread().getName()), call.toString(), numBytes).tag("methodCall").debug();
            }
          } else {
            //
            ...
            if (LogGlobal.isDebugEnabled()) {
              ...
              LOG.responding_wrote_partial_bytes(String.valueOf(Thread.currentThread().getName()), call.toString(), numBytes).tag("methodCall").debug();
            }
          }
        ...
        if (error && call != null) {
          ...
          LOG.call_output_error(String.valueOf(Thread.currentThread().getName()), call.toString()).tag("methodCall").warn();
  ...
  public class Connection {
    ...
    public Connection(SocketChannel channel, long lastContact) {
      ...
      if (socketSendBufferSize != 0) {
        try {
          ...
          LOG.connection_unable_set_socket_send_buffer(socketSendBufferSize).warn();
        }
      }
    }   
    ...
    private void saslProcess(RpcSaslProto saslMessage)
      ...
      try {
        try {
          ...
          LOG.authentication_failed_for(this.toString() + ":"
        ...
        if (saslServer != null && saslServer.isComplete()) {
          if (LogGlobal.isDebugEnabled()) {
            ...
            LOG.sasl_server_context_established_negotiat(String.valueOf(saslServer.getNegotiatedProperty(Sasl.QOP))).tag("methodCall").debug();
          }
          user = getAuthorizedUgi(saslServer.getAuthorizationID());
          if (LogGlobal.isDebugEnabled()) {
            /* LOG.debug("SASL server successfully authenticated client: "+user) */
            LOG.sasl_server_successfully_authenticated_c(user.toString()).debug();
          }
          rpcMetrics.incrAuthenticationSuccesses();
          LOG.authentication_successful_for(user.toString());
    ...
    private RpcSaslProto processSaslToken(RpcSaslProto saslMessage)
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.have_read_input_token_size_for(String.valueOf(saslToken.length)).tag("methodCall").debug();
      }
    ...
    private RpcSaslProto buildSaslResponse(SaslState state, byte[] replyToken) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.will_send_token_size_from_saslserver(state.toString(), String.valueOf(((replyToken!=null)?replyToken.length:null))).tag("ternary-operator").debug();
      }
    ...
    private void doSaslReply(Message message) throws IOException {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.sending_sasl_message(message.toString()).debug();
      }
      ...
      while (true) {
        ...
        if (!connectionHeaderRead) {
          ...
          if (!RpcConstants.HEADER.equals(dataLengthBuffer)
            ...
            LOG.incorrect_header_version_mismatch_from_g(hostAddress, remotePort, version, String.valueOf(CURRENT_VERSION)).warn();
    ...
    private void unwrapPacketAndProcessRpcs(byte[] inBuf)
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.have_read_input_token_size_for(String.valueOf(inBuf.length)).tag("methodCall").debug();
      }
    ...
    private void processRpcRequest(RpcRequestHeaderProto header,
      ...
      if (rpcRequestClass == null) {
        ...
        LOG.unknown_rpc_kind_from_client(String.valueOf(header.getRpcKind()), getHostAddress()).tag("methodCall").warn();
      ...
      try { //Read the rpc request
        ...
        LOG.unable_read_call_parameters_for_client_c(getHostAddress(), String.valueOf(this.protocolName), String.valueOf(header.getRpcKind()), t.toString()).tag("methodCall").warn();
    ...
    private void processRpcOutOfBandRequest(RpcRequestHeaderProto header,
      ...
      if (callId == CONNECTION_CONTEXT_CALL_ID) {
        ...
        LOG.received_ping_message().debug();
      } else {
        throw new WrappedRpcServerException(
            RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER,
    ...
    private void authorizeConnection() throws WrappedRpcServerException {
      try {
        ...
        if (LogGlobal.isDebugEnabled()) {
          ...
          LOG.successfully_authorized(connectionContext.toString()).debug();
        }
        rpcMetrics.incrAuthorizationSuccesses();
      } catch (AuthorizationException ae) {
        /* LOG.info("Connection from "+this+" for protocol "+connectionContext.getProtocol()+" is unauthorized for user "+user) */
        LOG.connection_from_for_protocol_unauthorize(this.toString(), String.valueOf(connectionContext.getProtocol()), user.toString()).tag("methodCall").info();
    ...
    private synchronized void close() {
      ...
      try {socket.shutdownOutput();} catch(Exception e) {
        ...
        LOG.ignoring_socket_shutdown_exception(e.toString()).debug();
      }
      if (channel.isOpen()) {
  ...
  private class Handler extends Thread {
    ...
    public void run() {
      ...
      LOG.starting(String.valueOf(Thread.currentThread().getName())).tag("methodCall").debug();
      SERVER.set(Server.this);
      ...
      while (running) {
        try {
          ...
          if (LogGlobal.isDebugEnabled()) {
            ...
            LOG.for_rpckind(String.valueOf(Thread.currentThread().getName()), call.toString(), String.valueOf(call.rpcKind)).tag("methodCall").debug();
          }
          if (!call.connection.channel.isOpen()) {
            /* LOG.info(Thread.currentThread().getName()+": skipped "+call) */
            LOG.skipped(String.valueOf(Thread.currentThread().getName()), call.toString()).tag("methodCall").info();
            continue;
          }
          ...
          try {
            ...
            if (exceptionsHandler.isTerse(e.getClass())) {
              ...
              /* LOG.info(logMsg+": "+e) */
              LOG.logmsgexception(logMsg, e.toString()).info();
            } else if (e instanceof RuntimeException || e instanceof Error) {
              // These exception types indicate something is probably wrong
              // on the server side, as opposed to just a normal exceptional
              // result.
              /* LOG.warn(logMsg,e) */
              LOG.logmsgexception(logMsg, e.toString()).warn();
            } else {
              /* LOG.info(logMsg,e) */
              LOG.logmsgexception(logMsg, e.toString()).info();
            }
          ...
          synchronized (call.connection.responseQueue) {
            ...
            if (buf.size() > maxRespSize) {
              ...
              LOG.large_response_size_for_call(String.valueOf(buf.size()), String.valueOf(call.toString())).tag("methodCall").warn();
          ...
          LOG.caught_exception(String.valueOf(Thread.currentThread().getName()), e.toString()).tag("methodCall").info();
        }
      }
      /* LOG.debug(Thread.currentThread().getName()+": exiting") */
      LOG.exiting(String.valueOf(Thread.currentThread().getName())).tag("methodCall").debug();
    }
  ...
  private List<AuthMethod> getAuthMethods(SecretManager<?> secretManager,
    ...
    if (confAuthenticationMethod == AuthenticationMethod.TOKEN) {
      ...
      LOG.authentication_enabled_for_secret_manage(String.valueOf(AuthenticationMethod.TOKEN)).tag("methodCall").debug();
    ...
    LOG.server_accepts_auth_methods(authMethods.toString()).debug();
    return authMethods;
  }
  ...
  throws IOException {
    ...
    if (status == RpcStatusProto.SUCCESS) {
      ...
      try {
        ...
        LOG.error_serializing_call_response_for_call(call.toString(), t.toString()).warn();
  ...
  private void wrapWithSasl(ByteArrayOutputStream response, Call call)
    ...
    if (call.connection.saslServer != null) {
      ...
      if (LogGlobal.isDebugEnabled())
        ...
        LOG.adding_saslserver_wrapped_token_size_cal(String.valueOf(token.length)).tag("methodCall").debug();
      response.reset();
  ...
  private class ConnectionManager {
    ...
    Connection register(SocketChannel channel) {
      ...
      add(connection);
      if (LogGlobal.isDebugEnabled()) {
        /* LOG.debug("Server connection from "+connection+"; # active connections: "+size()+"; # queued calls: "+callQueue.size()) */
        LOG.server_connection_from_active_connection(connection.toString(), size(), String.valueOf(callQueue.size())).tag("methodCall").debug();
      }      
      return connection;
    }
    ...
    boolean close(Connection connection) {
      ...
      if (exists) {
        if (LogGlobal.isDebugEnabled()) {
          ...
          LOG.disconnecting_client_number_active_conne(String.valueOf(Thread.currentThread().getName()), connection.toString(), size()).tag("methodCall").debug();
        }
      ...
      TimerTask idleScanTask = new TimerTask(){
        ...
        public void run() {
          ...
          if (LogGlobal.isDebugEnabled()) {
            ...
            LOG.task_running(String.valueOf(Thread.currentThread().getName())).tag("methodCall").debug();
          }
          try {
            closeIdle(false);
          } finally {
