...
public class SequenceFile {
  private static final IoNamespace LOG = LoggerFactory.getLogger(IoNamespace.class, new SimpleLogger());
  ...
  public static class Reader implements java.io.Closeable {
    ...
    public synchronized void getCurrentValue(Writable val) 
      ...
      if (!blockCompressed) {
        ...
        if (valIn.read() > 0) {
          ...
          LOG.available_bytes(String.valueOf(valIn.available())).tag("methodCall").info();
        ...
        // Sanity check
        if ((valLength < 0) && LogGlobal.isDebugEnabled()) {
          /* LOG.debug(val+" is a zero-length value") */
          LOG.zero_length_value(val.toString()).debug();
        }
      }
    ...
    private void handleChecksumException(ChecksumException e)
      ...
      if (this.conf.getBoolean("io.skip.checksum.errors", false)) {
        ...
        LOG.bad_checksum_skipping_entries(getPosition()).tag("methodCall").warn();
      ...
      public int run(boolean deleteInput) throws IOException {
        ...
        while (!atEof) {
          ...
          if(LogGlobal.isDebugEnabled()) {
            ...
            LOG.flushing_segment(segments).debug();
          }
        ...
        do {
          ...
          if (numSegments <= factor) {
            ...
            if(LogGlobal.isDebugEnabled()) { 
              ...
              LOG.writing_intermediate_results(outputFile.toString()).debug();
            }
            Writer writer = cloneFileAttributes(
