...
public class GangliaContext31 extends GangliaContext {
  ...
  private static final /* LogLOG=LogFactory.getLog("org.apache.hadoop.util.GangliaContext31"); */
  ...
  public void init(String contextName, ContextFactory factory) {
    ...
    LOG.initializing_gangliacontext_for_ganglia_().debug();
    ...
    if (conf.get("slave.host.name") != null) {
      ...
      try {
        ...
        /* LOG.error(uhe) */
        LOG.exception(uhe.toString()).error();
    ...
    throws IOException
  {
    if (name == null) {
      ...
      LOG.metric_was_emitted_with_name().warn();
      return;
    } else if (value == null) {
      /* LOG.warn("Metric name "+name+" was emitted with a null value.") */
      LOG.metric_name_was_emitted_with_null(name).warn();
      return;
    } else if (type == null) {
      /* LOG.warn("Metric name "+name+", value "+value+" has no type.") */
      LOG.metric_name_value_has_type(name, value).warn();
      return;
    }

    /* LOG.debug("Emitting metric "+name+", type "+type+", value "+value+" from hostname"+hostName) */
    LOG.emitting_metric_type_value_from_hostname(name, type, value, hostName).debug();

    String units = getUnits(name);
    if (units == null) {
      /* LOG.warn("Metric name "+name+", value "+value+" had 'null' units") */
      LOG.metric_name_value_had_null_units(name, value).warn();
      units = "";
    }
