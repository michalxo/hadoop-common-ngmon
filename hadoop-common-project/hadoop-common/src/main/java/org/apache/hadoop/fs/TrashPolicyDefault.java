...
public class TrashPolicyDefault extends TrashPolicy {
  private static final /* LogLOG=LogFactory.getLog(TrashPolicyDefault.class); */
  ...
  public void initialize(Configuration conf, FileSystem fs, Path home) {
    ...
    this.emptierInterval = (long)(conf.getFloat(
        ...
        * MSECS_PER_MINUTE);
    /* LOG.info("Namenode trash configuration: Deletion interval = "+(this.deletionInterval/MSECS_PER_MINUTE)+" minutes, Emptier interval = "+(this.emptierInterval/MSECS_PER_MINUTE)+" minutes.") */
    LOG.namenode_trash_configuration_deletion_in((this.deletionInterval/MSECS_PER_MINUTE), (this.emptierInterval/MSECS_PER_MINUTE)).tag("mathExp").info();
   }
    ...
    for (int i = 0; i < 2; i++) {
      try {
        if (!fs.mkdirs(baseTrashPath, PERMISSION)) {      // create current
          ...
          LOG.cant_create_mkdir_trash_directory(baseTrashPath.toString()).warn();
          return false;
        }
      } catch (IOException e) {
        /* LOG.warn("Can't create trash directory: "+baseTrashPath) */
        LOG.cant_create_trash_directory(baseTrashPath.toString()).warn();
        cause = e;
        break;
      }
      try {
  ...
  public void createCheckpoint() throws IOException {
    ...
    LOG.created_trash_checkpoint(String.valueOf(checkpoint.toUri().getPath())).tag("methodCall").info();
  }
  ...
  public void deleteCheckpoint() throws IOException {
    ...
    for (int i = 0; i < dirs.length; i++) {
      ...
      try {
        ...
        LOG.unexpected_item_trash_ignoring(dir).warn();
        continue;
      }
      ...
      if ((now - deletionInterval) > time) {
        if (fs.delete(path, true)) {
          ...
          LOG.deleted_trash_checkpoint(dir).info();
        } else {
          /* LOG.warn("Couldn't delete checkpoint: "+dir+" Ignoring.") */
          LOG.couldnt_delete_checkpoint_ignoring(dir).warn();
        }
      }
    }
  }
  ...
  private class Emptier implements Runnable {
    ...
    Emptier(Configuration conf, long emptierInterval) throws IOException {
      ...
      if (emptierInterval > deletionInterval || emptierInterval == 0) {
        ...
        LOG.configured_checkpoint_interval_minutes_u((emptierInterval/MSECS_PER_MINUTE), (deletionInterval/MSECS_PER_MINUTE)).tag("mathExp").info();
        ...
        try {
          ...
          if (now >= end) {
            ...
            try {
              ...
              LOG.trash_cant_list_homes_sleeping(e.toString()).warn();
              continue;
            }
            ...
            for (FileStatus home : homes) {         // dump each trash
              ...
              try {
                ...
                LOG.trash_caught_skipping(e.toString(), String.valueOf(home.getPath())).tag("methodCall").warn();
              } 
            }
          }
        } catch (Exception e) {
          /* LOG.warn("RuntimeException during Trash.Emptier.run(): ",e) */
          LOG.runtimeexception_during_trash_emptier_ru(e.toString()).warn(); 
        }
      }
      try {
        fs.close();
        ...
        LOG.trash_cannot_close_filesystem(e.toString()).warn();
      }
    }
