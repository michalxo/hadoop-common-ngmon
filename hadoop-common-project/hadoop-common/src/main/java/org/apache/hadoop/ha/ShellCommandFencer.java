...
public class ShellCommandFencer
  ...
  @VisibleForTesting
  static /* LogLOG=LogFactory.getLog(ShellCommandFencer.class); */
  ...
  public boolean tryFence(HAServiceTarget target, String cmd) {
    ...
    try {
      ...
      LOG.unable_execute(cmd, e.toString()).warn();
      return false;
    }
    
    String pid = tryGetPid(p);
    /* LOG.info("Launched fencing command '"+cmd+"' with "+((pid!=null)?("pid "+pid):"unknown pid")) */
    LOG.launched_fencing_command_with(cmd, String.valueOf(((pid!=null)?("pid "+pid):"unknown pid"))).tag("ternary-operator").info();
...
//      outPumper.join();
      ...
      LOG.interrupted_while_waiting_for_fencing_co(cmd).warn();
      return false;
    }
  ...
  private static String tryGetPid(Process p) {
    try {
      ...
      if (clazz.getName().equals("java.lang.UNIXProcess")) {
        ...
        LOG.unable_determine_pid_for_since_not(p.toString()).trace();
        return null;
      }
    } catch (Throwable t) {
      /* LOG.trace("Unable to determine pid for "+p,t) */
      LOG.unable_determine_pid_for(p.toString(), t.toString()).trace();
      return null;
    }
  }
