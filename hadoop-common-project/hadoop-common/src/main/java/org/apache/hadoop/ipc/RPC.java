...
public class RPC {
  ...
  interface RpcInvoker {   
    ...
    private static IpcNamespace LOG = LoggerFactory.getLogger(IpcNamespace.class, new SimpleLogger());
  /**
  ...
  static Class<?>[] getSuperInterfaces(Class<?>[] childInterfaces) {
    ...
    for (Class<?> childInterface : childInterfaces) {
      if (VersionedProtocol.class.isAssignableFrom(childInterface)) {
        ...
        LOG.interface_ignored_because_does_not_exten(childInterface.toString()).warn();
      }
    }
  ...
  public static <T> ProtocolProxy<T> waitForProtocolProxy(Class<T> protocol,
    ...
    while (true) {
      try {
        ...
        LOG.server_not_available_yet_zzzzz(addr.toString()).info();
        ioe = se;
      } catch(SocketTimeoutException te) {  // namenode is busy
        /* LOG.info("Problem connecting to server: "+addr) */
        LOG.problem_connecting_server(addr.toString()).info();
        ioe = te;
      } catch(NoRouteToHostException nrthe) { // perhaps a VIP is failing over
        /* LOG.info("No route to host for server: "+addr) */
        LOG.route_host_for_server(addr.toString()).info();
        ioe = nrthe;
      }
      // check if timed out
  ...
  public static void stopProxy(Object proxy) {
    ...
    try {
      ...
      LOG.closing_proxy_invocation_handler_caused_(e.toString()).error();
    } catch (IllegalArgumentException e) {
      /* LOG.error("RPC.stopProxy called on non proxy: class="+proxy.getClass().getName(),e) */
      LOG.rpc_stopproxy_called_non_proxy_class(String.valueOf(proxy.getClass().getName()), e.toString()).tag("methodCall").error();
    }
  ...
  public abstract static class Server extends org.apache.hadoop.ipc.Server {
   ...
   void registerProtocolAndImpl(RpcKind rpcKind, Class<?> protocolClass, 
     ...
     try {
       ...
       LOG.protocol_not_registered_cannot_get_proto(protocolClass.toString()).warn();
       return;
     }
     ...
     LOG.rpckind_protocol_name_version_protocolim(rpcKind.toString(), protocolName, version, String.valueOf(protocolImpl.getClass().getName()), String.valueOf(protocolClass.getName())).tag("methodCall").debug();
   }
   ...
   VerProtocolImpl getHighestSupportedProtocol(RpcKind rpcKind, 
     ...
     if (LogGlobal.isDebugEnabled()) {
       ...
       LOG.size_protomap_for(rpcKind.toString(), getProtocolImplMap(rpcKind).size()).tag("methodCall").debug();
     }
     for (Map.Entry<ProtoNameVer, ProtoClassProtoImpl> pv : 
           getProtocolImplMap(rpcKind).entrySet()) {
       if (pv.getKey().protocol.equals(protocolName)) {
