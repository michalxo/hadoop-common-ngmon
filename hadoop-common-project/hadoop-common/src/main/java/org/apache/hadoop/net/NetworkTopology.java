...
public class NetworkTopology {
  ...
  public static final /* LogLOG=LogFactory.getLog(NetworkTopology.class); */
  ...
  public void add(Node node) {
    ...
    try {
      if ((depthOfAllLeaves != -1) && (depthOfAllLeaves != newDepth)) {
        ...
        LOG.error_cant_add_leaf_node_depth_topology(String.valueOf(NodeBase.getPath(node)), newDepth, oldTopoStr).tag("methodCall").error();
      ...
      if (clusterMap.add(node)) {
        ...
        LOG.adding_new_node(String.valueOf(NodeBase.getPath(node))).tag("methodCall").info();
        if (rack == null) {
          numOfRacks++;
        }
        if (!(node instanceof InnerNode)) {
      ...
      if(LogGlobal.isDebugEnabled()) {
        ...
        LOG.networktopology_became(String.valueOf(this.toString())).tag("methodCall").debug();
      }
    } finally {
  ...
  public void remove(Node node) {
    ...
    LOG.removing_node(String.valueOf(NodeBase.getPath(node))).tag("methodCall").info();
    netlock.writeLock().lock();
    try {
  ...
  public int getDistance(Node node1, Node node2) {
    ...
    if (n1==null) {
      ...
      LOG.cluster_does_not_contain_node(String.valueOf(NodeBase.getPath(node1))).tag("methodCall").warn();
      return Integer.MAX_VALUE;
    }
    if (n2==null) {
      /* LOG.warn("The cluster does not contain node: "+NodeBase.getPath(node2)) */
      LOG.cluster_does_not_contain_node(String.valueOf(NodeBase.getPath(node2))).tag("methodCall").warn();
      return Integer.MAX_VALUE;
    }
    return dis+2;
  }
