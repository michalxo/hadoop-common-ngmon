...
public class SecurityUtil {
  public static final /* LogLOG=LogFactory.getLog(SecurityUtil.class); */
  ...
  public static void setTokenService(Token<?> token, InetSocketAddress addr) {
    ...
    if (token != null) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.acquired_token(token.toString()).debug();  // Token#toString() prints service
      }
    } else {
      /* LOG.warn("Failed to get token for service "+service) */
      LOG.failed_get_token_for_service(service.toString()).warn();
    }
  }
  ...
  public static <T> T doAsLoginUserOrFatal(PrivilegedAction<T> action) { 
    if (UserGroupInformation.isSecurityEnabled()) {
      ...
      try { 
        ...
        LOG.exception_while_getting_login_user(e.toString()).fatal();
        e.printStackTrace();
