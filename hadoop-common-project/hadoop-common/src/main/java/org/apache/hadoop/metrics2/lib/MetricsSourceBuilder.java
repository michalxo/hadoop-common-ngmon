...
public class MetricsSourceBuilder {
  private static final /* LogLOG=LogFactory.getLog(MetricsSourceBuilder.class); */
  ...
  private MetricsRegistry initRegistry(Object source) {
    ...
    for (Field field : ReflectionUtils.getDeclaredFieldsIncludingInherited(cls)) {
      ...
      try {
        ...
        LOG.error_accessing_field(field.toString(), e.toString()).warn();
        continue;
      }
    }
  ...
  private void add(Object source, Field field) {
    for (Annotation annotation : field.getAnnotations()) {
      ...
      try {
        ...
        LOG.error_accessing_field_annotated_with(field.toString(), annotation.toString(), e.toString()).warn();
        continue;
      }
