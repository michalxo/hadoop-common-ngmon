...
abstract public class FSInputChecker extends FSInputStream {
  public static final /* LogLOG=LogFactory.getLog(FSInputChecker.class); */
    ...
    do {
      ...
      try {
        ...
        retry = false;
          ...
          LOG.found_checksum_error(off, (off+read), StringUtils.byteToHexString(b,off,off+read), ce.toString()).tag("mathExp").tag("methodCall").info();
          if (retriesLeft == 0) {
            throw ce;
          }
