...
public class S3FileSystem extends FileSystem {
  ...
  public boolean rename(Path src, Path dst) throws IOException {
    ...
    boolean debugEnabled = LogGlobal.isDebugEnabled();
    if (srcINode == null) {
    ...
    if (srcIsFile) {
      ...
      if (destExists) {
        if (destIsDir) {
          ...
          if (debugEnabled) {
            ...
            LOG.copying_src_file_under_dest_dir(debugPreamble, absoluteDst.toString()).debug();
          }
          ...
          if (debugEnabled) {
            ...
            LOG.copying_file_onto_file_outcome(debugPreamble, renamingOnToSelf).debug();
          }
        ...
        if (debugEnabled) {
          ...
          LOG.copying_file_onto_file(debugPreamble).debug();
        }
      }
    } else {
      ...
      if (destExists) {
        if (!destIsDir) {
          ...
          if (debugEnabled) {
            ...
            LOG.returning_false_src_directory_but_not(debugPreamble).debug();
          }
          return false;
        } else {
          ...
          if (debugEnabled) {
            ...
            LOG.copying_src_dir_under_dest_dir(debugPreamble, absoluteDst.toString()).debug();
          }
        }
      }
      ...
      if (absoluteSrc.equals(absoluteDst)) {
        ...
        if (debugEnabled) {
          ...
          LOG.dest_source_isdir_failing(debugPreamble).debug();
        }
        return false;
      }
      if (absoluteDst.toString().startsWith(absoluteSrc.toString() + "/")) {
        ...
        if (debugEnabled) {
          ...
          LOG.dst_equal_under_src_dir_failing(debugPreamble).debug();
        }
        return false;
      }
    }
