...
public enum SignalLogger {
  ...
  private boolean registered = false;
    private static final UtilNamespace LOG = LoggerFactory.getLogger(UtilNamespace.class, new SimpleLogger());
  /**
   * Our signal handler.
   */
...
//      this.log = log;
    ...
    public void handle(Signal signal) {
      ...
      LOG.received_signal_sig(String.valueOf(signal.getNumber()), String.valueOf(signal.getName())).tag("methodCall").error();
  ...
  public void register(final Log log) {
    ...
    for (String signalName : SIGNALS) {
      try {
        ...
        /* LOG.debug(e) */
        LOG.exception(e.toString()).debug();
      }
    }
    bld.append("]");
    /* LOG.info(bld.toString()) */
    LOG.bldmethodcall(String.valueOf(bld.toString())).tag("methodCall").info();
  }
}
