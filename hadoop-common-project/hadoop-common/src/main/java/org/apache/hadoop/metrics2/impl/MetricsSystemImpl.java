...
public class MetricsSystemImpl extends MetricsSystem implements MetricsSource {
  ...
  static final /* LogLOG=LogFactory.getLog(MetricsSystemImpl.class); */
  ...
  public synchronized MetricsSystem init(String prefix) {
    if (monitoring && !DefaultMetricsSystem.inMiniClusterMode()) {
      ...
      LOG.metrics_system_already_initialized(String.valueOf(this.prefix)).tag("methodCall").warn();
      return this;
    }
    ...
    if (monitoring) {
      ...
      LOG.metrics_system_started_again(String.valueOf(this.prefix)).tag("methodCall").info();
      return this;
    }
    switch (initMode()) {
      case NORMAL:
        try { start(); }
          ...
          LOG.metrics_system_not_started(String.valueOf(e.getMessage())).tag("methodCall").warn();
          /* LOG.debug("Stacktrace: ",e) */
          LOG.stacktrace(e.toString()).debug();
        }
        break;
      case STANDBY:
        /* LOG.info(prefix+" metrics system started in standby mode") */
        LOG.metrics_system_started_standby_mode(prefix).info();
    }
    initSystemMBean();
    return this;
  }
  ...
  public synchronized void start() {
    ...
    if (monitoring) {
      /* LOG.warn(prefix+" metrics system already started!",newMetricsException("Illegal start")) */
      LOG.metrics_system_already_started(prefix, new MetricsException("Illegal start").toString()).tag("methodCall").warn();
      return;
    }
    ...
    LOG.metrics_system_started(prefix).info();
  ...
  public synchronized void stop() {
    if (!monitoring && !DefaultMetricsSystem.inMiniClusterMode()) {
      /* LOG.warn(prefix+" metrics system not yet started!",newMetricsException("Illegal stop")) */
      LOG.metrics_system_not_yet_started(prefix, new MetricsException("Illegal stop").toString()).tag("methodCall").warn();
      return;
    }
    if (!monitoring) {
      // in mini cluster mode
      /* LOG.info(prefix+" metrics system stopped (again)") */
      LOG.metrics_system_stopped_again(prefix).info();
      return;
    }
    for (Callback cb : callbacks) cb.preStop();
    /* LOG.info("Stopping "+prefix+" metrics system...") */
    LOG.stopping_metrics_system(prefix).info();
    stopTimer();
    stopSources();
    stopSinks();
    clearConfigs();
    ...
    LOG.metrics_system_stopped(prefix).info();
  ...
  T register(String name, String desc, T source) {
    ...
    LOG.finalnamefinalde(finalName, finalDesc).debug();
    if (monitoring) {
  ...
  void registerSource(String name, String desc, MetricsSource source) {
    ...
    LOG.registered_source(name).debug();
  }
  ...
  T register(final String name, final String description, final T sink) {
    ...
    LOG.namedescription(name, description).debug();
    if (allSinks.containsKey(name)) {
      /* LOG.warn("Sink "+name+" already exists!") */
      LOG.sink_already_exists(name).warn();
      return sink;
    }
        ...
        new InvocationHandler() {
          ...
          public Object invoke(Object proxy, Method method, Object[] args)
            ...
            try {
              ...
              LOG.caught_exception_callback(String.valueOf(method.getName()), e.toString()).tag("methodCall").warn();
            }
            return null;
          }
        }));
  ...
  private synchronized void startTimer() {
    if (timer != null) {
      ...
      LOG.metrics_system_timer_already_started(prefix).warn();
      return;
    }
    logicalTime = 0;
    ...
    LOG.scheduled_snapshot_period_second(period).info();
  }
  ...
  private void snapshotMetrics(MetricsSourceAdapter sa,
    ...
    LOG.snapshotted_source(String.valueOf(sa.name())).tag("methodCall").debug();
  }
  ...
  private synchronized void stopTimer() {
    if (timer == null) {
      ...
      LOG.metrics_system_timer_already_stopped(prefix).warn();
      return;
    }
    timer.cancel();
    timer = null;
  ...
  private synchronized void stopSources() {
    for (Entry<String, MetricsSourceAdapter> entry : sources.entrySet()) {
      ...
      LOG.stopping_metrics_source_class(String.valueOf(entry.getKey()), String.valueOf(sa.source().getClass())).tag("methodCall").debug();
      sa.stop();
    }
    sysSource.stop();
    sources.clear();
  ...
  private synchronized void stopSinks() {
    for (Entry<String, MetricsSinkAdapter> entry : sinks.entrySet()) {
      ...
      LOG.stopping_metrics_sink_class(String.valueOf(entry.getKey()), String.valueOf(sa.sink().getClass())).tag("methodCall").debug();
      sa.stop();
    }
    sinks.clear();
  }
  ...
  private synchronized void configureSinks() {
    ...
    for (Entry<String, MetricsConfig> entry : sinkConfigs.entrySet()) {
      ...
      try {
        ...
        LOG.error_creating_sink(sinkName, e.toString()).warn();
      }
    }
  ...
  static String getHostname() {
    try {
      ...
      LOG.error_getting_localhost_name_using_local(e.toString()).error();
    }
    return "localhost";
  }
  ...
  public synchronized boolean shutdown() {
    ...
    LOG.refcount(refCount).debug();
    if (refCount <= 0) {
      /* LOG.debug("Redundant shutdown",newThrowable()) */
      LOG.redundant_shutdown(new Throwable().toString()
      ).tag("methodCall").debug();
    ...
    if (monitoring) {
      try { stop(); }
        ...
        LOG.error_stopping_metrics_system(e.toString()).warn();
      }
    }
    allSources.clear();
    allSinks.clear();
    ...
    LOG.metrics_system_shutdown_complete(prefix).info();
    return true;
  }
  ...
  private InitMode initMode() {
    ...
    LOG.from_system_property(String.valueOf(System.getProperty(MS_INIT_MODE_KEY))).tag("methodCall").debug();
    /* LOG.debug("from environment variable: "+System.getenv(MS_INIT_MODE_KEY)) */
    LOG.from_environment_variable(String.valueOf(System.getenv(MS_INIT_MODE_KEY))).tag("methodCall").debug();
