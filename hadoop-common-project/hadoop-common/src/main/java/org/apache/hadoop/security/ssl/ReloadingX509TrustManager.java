...
public final class ReloadingX509TrustManager
  ...
  private static final /* LogLOG=LogFactory.getLog(ReloadingX509TrustManager.class); */
  ...
  throws IOException, GeneralSecurityException {
    ...
    try {
      ...
      LOG.loaded_truststore(file.toString()).debug();
    } finally {
      in.close();
    }
    ...
    while (running) {
      ...
      if (running && needsReload()) {
        try {
          ...
          /* LOG.warn("Could not load truststore (keep using existing one) : "+ex.toString(),ex) */
          LOG.could_not_load_truststore_keep_using(String.valueOf(ex.toString()), ex.toString()).tag("methodCall").warn();
        }
      }
    }
  }
