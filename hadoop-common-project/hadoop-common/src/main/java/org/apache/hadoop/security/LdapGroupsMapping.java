...
public class LdapGroupsMapping
  ...
  private static final /* LogLOG=LogFactory.getLog(LdapGroupsMapping.class); */
  ...
  public synchronized List<String> getGroups(String user) throws IOException {
    ...
    try {
      ...
      LOG.connection_closed_will_try_reconnect().warn();
    } catch (NamingException e) {
      /* LOG.warn("Exception trying to get groups for user "+user,e) */
      LOG.exception_trying_get_groups_for_user(user, e.toString()).warn();
      return emptyResults;
    }
    ...
    while (retryCount ++ < RECONNECT_RETRY_COUNT) {
      ...
      try {
        ...
        LOG.connection_being_closed_reconnecting_fai(retryCount).warn();
      } catch (NamingException e) {
        /* LOG.warn("Exception trying to get groups for user "+user,e) */
        LOG.exception_trying_get_groups_for_user(user, e.toString()).warn();
        return emptyResults;
