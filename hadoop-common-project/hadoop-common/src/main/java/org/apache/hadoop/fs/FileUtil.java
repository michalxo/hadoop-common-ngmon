...
public class FileUtil {
  ...
  private static final /* LogLOG=LogFactory.getLog(FileUtil.class); */
  ...
  private static boolean deleteImpl(final File f, final boolean doLog) {
    ...
    if (doLog && ex) {
      ...
      LOG.failed_delete_file_dir_still_exists(String.valueOf(f.getAbsolutePath())).tag("methodCall").warn();
    }
    return !ex;
  }
  ...
  private static void unTarUsingJava(File inFile, File untarDir,
    ...
    try {
      ...
      IOUtils.cleanup(null, tis, inputStream);
    }
  }
  ...
  public static int symLink(String target, String linkname) throws IOException{
    ...
    if (Shell.WINDOWS && !Shell.isJava7OrAbove() && targetFile.isFile()) {
      try {
        ...
        LOG.fileutil_symlink_windows_java_copying_fi(target, linkname).warn();

        if (!linkFile.getParentFile().exists()) {
          /* LOG.warn("Parent directory "+linkFile.getParent()+" does not exist.") */
          LOG.parent_directory_does_not_exist(String.valueOf(linkFile.getParent())).tag("methodCall").warn();
          return 1;
        } else {
        ...
        LOG.fileutil_symlink_failed_copy_file_with(String.valueOf(ex.getMessage())).tag("methodCall").warn();
    ...
    try {
      ...
      if (Shell.WINDOWS && returnVal == SYMLINK_NO_PRIVILEGE) {
        ...
        LOG.fail_create_symbolic_links_windows_defau().warn();
      } else if (returnVal != 0) {
        /* LOG.warn("Command '"+StringUtils.join(" ",cmd)+"' failed "+returnVal+" with: "+ec.getMessage()) */
        LOG.command_failed_with(StringUtils.join(" ",cmd), returnVal, String.valueOf(ec.getMessage())).tag("methodCall").warn();
      }
      return returnVal;
    } catch (IOException e) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.error_while_create_symlink_exception(linkname, target, StringUtils.stringifyException(e)).tag("methodCall").debug();
      }
      throw e;
    }
  ...
  public static int chmod(String filename, String perm, boolean recursive)
    ...
    try {
      ...
      if(LogGlobal.isDebugEnabled()) {
        ...
        LOG.error_while_changing_permission_exceptio(filename, StringUtils.stringifyException(e)).tag("methodCall").debug();
      }
    }
    return shExec.getExitCode();
  }
  ...
  public static String createJarWithClassPath(String inputClassPath, Path pwd,
    ...
    if (!workingDir.mkdirs()) {
      ...
      LOG.mkdirs_false_for_execution_will_continue(workingDir.toString()).debug();
    }
    ...
    try {
      ...
      IOUtils.cleanup(null, jos, bos, fos);
    }
