...
public class Progress {
  private static final /* LogLOG=LogFactory.getLog(Progress.class); */
  ...
  public synchronized Progress addPhase(float weightage) {
    ...
    if (sum > 1.0) {
      ...
      LOG.sum_weightages_can_not_more_than(sum).warn();
    }
  ...
  public synchronized void set(float progress) {
    if (Float.isNaN(progress)) {
      progress = 0;
      /* LOG.warn("Illegal progress value found, progress is Float.NaN. "+"Progress will be changed to 0") */
      LOG.illegal_progress_value_found_progress_fl().warn();
    }
    else if (progress == Float.NEGATIVE_INFINITY) {
      progress = 0;
      /* LOG.warn("Illegal progress value found, progress is "+"Float.NEGATIVE_INFINITY. Progress will be changed to 0") */
      LOG.illegal_progress_value_found_progress_fl().warn();
    }
    else if (progress < 0) {
      progress = 0;
      /* LOG.warn("Illegal progress value found, progress is less than 0."+" Progress will be changed to 0") */
      LOG.illegal_progress_value_found_progress_le().warn();
    }
    else if (progress > 1) {
      progress = 1;
      /* LOG.warn("Illegal progress value found, progress is larger than 1."+" Progress will be changed to 1") */
      LOG.illegal_progress_value_found_progress_la().warn();
    }
    else if (progress == Float.POSITIVE_INFINITY) {
      progress = 1;
      /* LOG.warn("Illegal progress value found, progress is "+"Float.POSITIVE_INFINITY. Progress will be changed to 1") */
      LOG.illegal_progress_value_found_progress_fl().warn();
    }
