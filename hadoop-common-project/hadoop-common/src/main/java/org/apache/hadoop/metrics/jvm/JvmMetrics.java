...
public class JvmMetrics implements Updater {
    ...
    private static /* Loglog=LogFactory.getLog(JvmMetrics.class); */
    ...
    public synchronized static JvmMetrics init(String processName, String sessionId,
      String recordName) {
        if (theInstance != null) {
            ...
            LOG.cannot_initialize_jvm_metrics_with_proce(processName, sessionId).info();
        }
        else {
            /* log.info("Initializing JVM Metrics with processName="+processName+", sessionId="+sessionId) */
            LOG.initializing_jvm_metrics_with_processnam(processName, sessionId).info();
