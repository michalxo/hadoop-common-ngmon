...
public class NativeCodeLoader {
  ...
  private static final /* LogLOG=LogFactory.getLog(NativeCodeLoader.class); */
  ...
  static {
    ...
    if(LogGlobal.isDebugEnabled()) {
      ...
      LOG.trying_load_custom_built_native_hadoop().debug();
    }
    try {
      System.loadLibrary("hadoop");
      /* LOG.debug("Loaded the native-hadoop library") */
      LOG.loaded_native_hadoop_library().debug();
      nativeCodeLoaded = true;
    } catch (Throwable t) {
      // Ignore failure to load
      if(LogGlobal.isDebugEnabled()) {
        /* LOG.debug("Failed to load native-hadoop with error: "+t) */
        LOG.failed_load_native_hadoop_with_error(t.toString()).debug();
        /* LOG.debug("java.library.path="+System.getProperty("java.library.path")) */
        LOG.java_library_path(String.valueOf(System.getProperty("java.library.path"))).tag("methodCall").debug();
      }
    }
    
    if (!nativeCodeLoaded) {
      /* LOG.warn("Unable to load native-hadoop library for your platform... "+"using builtin-java classes where applicable") */
      LOG.unable_load_native_hadoop_library_for_us().warn();
    }
  }
