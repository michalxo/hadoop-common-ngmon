...
class TeraScheduler {
  ...
  private static final /* LogLOG=LogFactory.getLog(TeraScheduler.class); */
  ...
  Host pickBestHost() {
    ...
    if (result != null) {
      ...
      LOG.picking(result.toString()).debug();
    }
    return result;
  }
  ...
  void pickBestSplits(Host host) {
    ...
    for(Split cur: host.splits) {
      ...
      LOG.examine(String.valueOf(cur.filename), String.valueOf(cur.locations.size())).tag("methodCall").debug();
      int i = 0;
    ...
    for(int i=0; i < tasksToPick; ++i) {
      if (best[i] != null) {
        ...
        LOG.best(String.valueOf(best[i].filename)).debug();
