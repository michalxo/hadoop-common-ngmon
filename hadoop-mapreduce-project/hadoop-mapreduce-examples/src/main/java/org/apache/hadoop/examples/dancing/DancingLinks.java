...
public class DancingLinks<ColumnName> {
  private static final /* LogLOG=LogFactory.getLog(DancingLinks.class.getName()); */
  ...
  private void coverColumn(ColumnHeader<ColumnName> col) {
    ...
    LOG.cover(String.valueOf(col.head.name)).tag("methodCall").debug();
    // remove the column
  ...
  private void uncoverColumn(ColumnHeader<ColumnName> col) {
    ...
    LOG.uncover(String.valueOf(col.head.name)).tag("methodCall").debug();
