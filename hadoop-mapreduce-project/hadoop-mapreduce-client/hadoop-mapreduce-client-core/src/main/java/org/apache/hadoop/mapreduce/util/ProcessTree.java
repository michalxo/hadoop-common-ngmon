...
public class ProcessTree {
  ...
  private static final /* LogLOG=LogFactory.getLog(ProcessTree.class); */
  ...
  private static boolean isSetsidSupported() {
    ...
    try {
      ...
      LOG.setsid_not_available_this_machine_not().warn();
      setsidSupported = false;
    } finally { // handle the exit code
      /* LOG.info("setsid exited with exit code "+shexec.getExitCode()) */
      LOG.setsid_exited_with_exit_code(String.valueOf(shexec.getExitCode())).tag("methodCall").info();
    }
  ...
  private static void sendSignal(String pid, int signalNum, String signalName) {
    ...
    try {
      ...
      LOG.error_executing_shell_command(ioe.toString()).warn();
    } finally {
      if (pid.startsWith("-")) {
        /* LOG.info("Sending signal to all members of process group "+pid+": "+signalName+". Exit code "+shexec.getExitCode()) */
        LOG.sending_signal_all_members_process_group(pid, signalName, String.valueOf(shexec.getExitCode())).tag("methodCall").info();
      } else {
        /* LOG.info("Signaling process "+pid+" with "+signalName+". Exit code "+shexec.getExitCode()) */
        LOG.signaling_process_with_exit_code(pid, signalName, String.valueOf(shexec.getExitCode())).tag("methodCall").info();
      }
    }
  }
  ...
  private static void sigKillInCurrentThread(String pid, boolean isProcessGroup,
    ...
    if (isProcessGroup || ProcessTree.isAlive(pid)) {
      try {
        ...
        LOG.thread_sleep_interrupted().warn();
      }
      if(isProcessGroup) {
  ...
  public static boolean isAlive(String pid) {
    ...
    try {
      ...
      LOG.error_executing_shell_command(String.valueOf(Arrays.toString(shexec.getExecString())), ioe.toString()).tag("methodCall").warn();
      return false;
    }
