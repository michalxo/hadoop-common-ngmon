...
public class JobEndNotifier {
  private static final /* LogLOG=LogFactory.getLog(JobEndNotifier.class.getName()); */
    ...
    if (notification != null) {
      do {
        try {
          ...
          LOG.notification_error(String.valueOf(notification.getUri()), ioex.toString()).tag("methodCall").error();
        }
        catch (Exception ex) {
          /* LOG.error("Notification error ["+notification.getUri()+"]",ex) */
          LOG.notification_error(String.valueOf(notification.getUri()), ex.toString()).tag("methodCall").error();
        }
        try {
          ...
          LOG.notification_retry_error(notification.toString(), iex.toString()).error();
        }
