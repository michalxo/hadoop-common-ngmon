...
public class Merger {  
  private static final /* LogLOG=LogFactory.getLog(Merger.class); */
  ...
  extends PriorityQueue<Segment<K, V>> implements RawKeyValueIterator {
    ...
    throws IOException {
      ...
      for (Path file : inputs) {
        ...
        LOG.mergeq_adding(file.toString()).debug();
    ...
    RawKeyValueIterator merge(Class<K> keyClass, Class<V> valueClass,
      ...
      LOG.merging_sorted_segments(String.valueOf(segments.size())).tag("methodCall").info();
      ...
      do {
        ...
        if (numSegments <= factor) {
          ...
          LOG.down_last_merge_pass_with_segments_bytes(numSegments, (totalBytes-totalBytesProcessed)).tag("mathExp").info();
          return this;
        } else {
          /* LOG.info("Merging "+segmentsToMerge.size()+" intermediate segments out of a total of "+(segments.size()+segmentsToMerge.size())) */
          LOG.merging_intermediate_segments_out_total(String.valueOf(segmentsToMerge.size()), (segments.size()+segmentsToMerge.size())).tag("mathExp").tag("methodCall").info();
