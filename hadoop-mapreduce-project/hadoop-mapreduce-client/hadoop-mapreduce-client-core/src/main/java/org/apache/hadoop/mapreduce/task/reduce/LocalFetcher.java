...
class LocalFetcher<K,V> extends Fetcher<K, V> {
  ...
  private static final /* LogLOG=LogFactory.getLog(LocalFetcher.class); */
  ...
  private void doCopy(Set<TaskAttemptID> maps) throws IOException {
    ...
    while (iter.hasNext()) {
      ...
      LOG.localfetcher_going_fetch(id, map.toString()).debug();
  ...
  private boolean copyMapOutput(TaskAttemptID mapTaskId) throws IOException {
    ...
    LOG.localfetcher_about_shuffle_output_map_de(id, String.valueOf(mapOutput.getMapId()), decompressedLength, compressedLength, String.valueOf(mapOutput.getDescription())).tag("methodCall").info();
    ...
    try {
      ...
      try {
        ...
        LOG.ioexception_closing_inputstream_from_map(String.valueOf(ioe.toString())).tag("methodCall").warn();
      }
    }
