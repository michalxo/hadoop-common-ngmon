...
class SortedRanges implements Writable{
  ...
  private static final /* LogLOG=LogFactory.getLog(SortedRanges.class); */
  ...
  synchronized void add(Range range){
    ...
    if(headSet.size()>0) {
      ...
      LOG.previousrange(previousRange.toString()).debug();
    ...
    while(tailSetIt.hasNext()) {
      ...
      LOG.nextrange_startindex_endindex(nextRange.toString(), startIndex, endIndex).debug();
    ...
    if(headSet.size()>0) {
      ...
      if(startIndex<previousRange.getEndIndex()) {
        ...
        if(ranges.remove(previousRange)) {
          ...
          LOG.removed_previousrange(previousRange.toString()).debug();
        }
  ...
  private void add(long start, long end) {
    if(end>start) {
      ...
      LOG.added(recRange.toString()).debug();
    }
  }
  ...
  static class SkipRangeIterator implements Iterator<Long> {
    ...
    private void doNext() {
      ...
      LOG.currentindex(next, range.toString()).debug();
      skipIfInRange();
    ...
    private void skipIfInRange() {
      if(next>=range.getStartIndex() && 
        ...
        LOG.skipping_index(next, String.valueOf(range.getEndIndex())).tag("methodCall").warn();
        next = range.getEndIndex();
