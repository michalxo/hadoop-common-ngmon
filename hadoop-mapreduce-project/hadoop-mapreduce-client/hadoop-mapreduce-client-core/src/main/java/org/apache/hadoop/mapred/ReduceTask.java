...
public class ReduceTask extends Task {
  ...
  private static final /* LogLOG=LogFactory.getLog(ReduceTask.class.getName()); */
  ...
  private class SkippingReduceValuesIterator<KEY,VALUE> 
     ...
     private void mayBeSkip() throws IOException {
       ...
       if(!hasNext) {
         ...
         LOG.further_groups_got_skipped().warn();
         return;
       }
       grpIndex++;
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    LOG.using_shuffleconsumerplugin(shuffleConsumerPlugin.toString()).info();
  ...
  void closeQuietly(RecordWriter<OUTKEY, OUTVALUE> c, Reporter r) {
    if (c != null) {
      try {
        ...
        LOG.exception_closing(c.toString(), e.toString()).info();
      }
    }
  }
}
