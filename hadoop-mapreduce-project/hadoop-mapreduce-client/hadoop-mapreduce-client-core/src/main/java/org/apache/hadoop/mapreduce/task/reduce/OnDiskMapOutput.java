...
class OnDiskMapOutput<K, V> extends MapOutput<K, V> {
  private static final /* LogLOG=LogFactory.getLog(OnDiskMapOutput.class); */
  ...
  public void shuffle(MapHost host, InputStream input,
    ...
    try {
      ...
      LOG.read_bytes_from_map_output_for(String.valueOf(compressedLength-bytesLeft), getMapId().toString()).tag("mathExp").tag("methodCall").info();

      disk.close();
    } catch (IOException ioe) {
      // Close the streams
      IOUtils.cleanup(null, input, disk);
  ...
  public void abort() {
    try {
      ...
      LOG.failure_clean(tmpOutputPath.toString(), ie.toString()).info();
    }
  }
