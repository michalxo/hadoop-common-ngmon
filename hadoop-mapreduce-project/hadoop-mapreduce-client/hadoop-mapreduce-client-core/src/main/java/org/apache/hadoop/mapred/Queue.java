...
class Queue implements Comparable<Queue>{
  ...
  private static final /* LogLOG=LogFactory.getLog(Queue.class); */
  ...
  JobQueueInfo getJobQueueInfo() {
    ...
    LOG.created_jobqinfo(String.valueOf(queueInfo.getQueueName())).tag("methodCall").debug();
  ...
  boolean isHierarchySameAs(Queue newState) {
    ...
    if(!(name.equals(newState.getName())) ) {
      ...
      LOG.current_name_not_equal(name, String.valueOf(newState.getName())).tag("methodCall").info();
      return false;
    }
    ...
    if (children == null || children.size() == 0) {
      if(newState.getChildren() != null && newState.getChildren().size() > 0) {
        ...
        LOG.has_added_children_refresh(newState.toString()).info();
        return false;
      }
      ...
      if (newState.getChildren() == null) {
        ...
        LOG.current_state_queue_has_but_new(getName(), String.valueOf(children.size())).tag("methodCall").fatal();
        return false;
      }
      ...
      if (childrenSize != newChildrenSize) {
        ...
        LOG.number_children_for_queue_newstate_which(String.valueOf(newState.getName()), newChildrenSize, childrenSize).tag("methodCall").fatal();
        return false;
      }
      ...
      while(itr1.hasNext()) {
        ...
        if(! (q.isHierarchySameAs(newq)) ) {
          ...
          LOG.queue_not_equal(String.valueOf(q.getName()), String.valueOf(newq.getName())).tag("methodCall").info();
          return false;
        }
      }
    }
