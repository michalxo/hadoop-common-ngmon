...
public class MergeManagerImpl<K, V> implements MergeManager<K, V> {
  ...
  private static final /* LogLOG=LogFactory.getLog(MergeManagerImpl.class); */
  ...
  public MergeManagerImpl(TaskAttemptID reduceId, JobConf jobConf, 
    ...
    LOG.mergermanager_memorylimit_maxsingleshuff(memoryLimit, maxSingleShuffleLimit, mergeThreshold, ioSortFactor, memToMemMergeOutputsThreshold).info();
  ...
  public synchronized MapOutput<K,V> reserve(TaskAttemptID mapId, 
    ...
    if (!canShuffleToMemory(requestedSize)) {
      ...
      LOG.shuffling_disk_since_greater_than_maxsin(mapId.toString(), requestedSize, maxSingleShuffleLimit).info();
    ...
    if (usedMemory > memoryLimit) {
      ...
      LOG.stalling_shuffle_since_usedmemory_greate(mapId.toString(), usedMemory, memoryLimit, commitMemory).debug(); 
      return null;
    }
    ...
    LOG.proceeding_with_shuffle_since_usedmemory(mapId.toString(), usedMemory, memoryLimit, commitMemory).debug(); 
  ...
  public synchronized void closeInMemoryFile(InMemoryMapOutput<K,V> mapOutput) { 
    ...
    LOG.closeinmemoryfile_map_output_size_inmemo(String.valueOf(mapOutput.getSize()), String.valueOf(inMemoryMapOutputs.size()), commitMemory, usedMemory).tag("methodCall").info();
    ...
    if (commitMemory >= mergeThreshold) {
      ...
      LOG.starting_inmemorymergers_merge_since_com(commitMemory, mergeThreshold, usedMemory).info();
  ...
  public synchronized void closeInMemoryMergedFile(InMemoryMapOutput<K,V> mapOutput) {
    ...
    LOG.closeinmemorymergedfile_size_inmemorymer(String.valueOf(mapOutput.getSize()), String.valueOf(inMemoryMergedMapOutputs.size())).tag("methodCall").info();
  }
  ...
  extends MergeThread<InMemoryMapOutput<K, V>, K, V> {
    ...
    public void merge(List<InMemoryMapOutput<K, V>> inputs) throws IOException {
      ...
      LOG.initiating_memory_memory_merge_with_segm(noInMemorySegments, mergeOutputSize).info();
      ...
      LOG.memory_memory_merge_files_memory_complet(reduceId.toString(), noInMemorySegments).info();
  ...
  private class InMemoryMerger extends MergeThread<InMemoryMapOutput<K,V>, K,V> {
    ...
    public void merge(List<InMemoryMapOutput<K,V>> inputs) throws IOException {
      ...
      try {
        ...
        LOG.initiating_memory_merge_with_segments(noInMemorySegments).info();
        ...
        LOG.merge_files_memory_complete_local_file_s(reduceId.toString(), noInMemorySegments, outputPath.toString(), String.valueOf(localFS.getFileStatus(outputPath).getLen())).tag("methodCall").info();
      } catch (IOException e) { 
  ...
  private class OnDiskMerger extends MergeThread<CompressAwarePath,K,V> {
    ...
    public void merge(List<CompressAwarePath> inputs) throws IOException {
      ...
      LOG.ondiskmerger_have_map_outputs_disk_trigg(String.valueOf(inputs.size())).tag("methodCall").info();
      ...
      LOG.finished_merging_map_output_files_disk_l(reduceId.toString(), String.valueOf(inputs.size()), approxOutputSize, outputPath.toString(), String.valueOf(localFS.getFileStatus(outputPath).getLen())).tag("methodCall").info();
    }
  }
  ...
  private RawKeyValueIterator finalMerge(JobConf job, FileSystem fs,
    ...
    LOG.finalmerge_called_with_memory_map_output(String.valueOf(inMemoryMapOutputs.size()), String.valueOf(onDiskMapOutputs.size())).tag("methodCall").info();
    ...
    if (inMemoryMapOutputs.size() > 0) {
      ...
      if (numMemDiskSegments > 0 &&
        ...
        LOG.merged_segments_bytes_disk_satisfy_reduc(numMemDiskSegments, inMemToDiskBytes).info();
        inMemToDiskBytes = 0;
        memDiskSegments.clear();
      } else if (inMemToDiskBytes != 0) {
        /* LOG.info("Keeping "+numMemDiskSegments+" segments, "+inMemToDiskBytes+" bytes in memory for "+"intermediate, on-disk merge") */
        LOG.keeping_segments_bytes_memory_for_interm(numMemDiskSegments, inMemToDiskBytes).info();
      }
    }
    ...
    for (CompressAwarePath file : onDisk) {
      ...
      LOG.disk_file_length(file.toString(), fileLength).debug();
    ...
    LOG.merging_files_bytes_from_disk(String.valueOf(onDisk.length), onDiskBytes).tag("methodCall").info();
    ...
    LOG.merging_segments_bytes_from_memory_into(String.valueOf(finalSegments.size()), inMemBytes).tag("methodCall").info();
    if (0 != onDiskBytes) {
