...
abstract class MergeThread<T,K,V> extends Thread {
  ...
  private static final /* LogLOG=LogFactory.getLog(MergeThread.class); */
  ...
  public void startMerge(Set<T> inputs) {
    if (!closed) {
      ...
      LOG.starting_merge_with_segments_while_ignor(getName(), String.valueOf(toMergeInputs.size()), String.valueOf(inputs.size())).tag("methodCall").info();
