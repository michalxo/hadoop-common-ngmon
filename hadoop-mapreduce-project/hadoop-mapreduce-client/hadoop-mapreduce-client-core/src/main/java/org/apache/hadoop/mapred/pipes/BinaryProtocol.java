...
class BinaryProtocol<K1 extends WritableComparable, V1 extends Writable,
  ...
  private static final /* LogLOG=LogFactory.getLog(BinaryProtocol.class.getName()); */
    ...
    public void run() {
      while (true) {
        try {
          ...
          LOG.handling_uplink_command(cmd).debug();
          if (cmd == MessageType.AUTHENTICATION_RESP.code) {
            ...
            LOG.message_received_before_authentication_c(cmd).warn();
            continue;
          ...
          LOG.exception(e.toString()).error();
          handler.failed(e);
          return;
        }
      }
  ...
  public void authenticate(String digest, String challenge)
    ...
    LOG.sending_authentication_req_digest_challe(digest, challenge).debug();
