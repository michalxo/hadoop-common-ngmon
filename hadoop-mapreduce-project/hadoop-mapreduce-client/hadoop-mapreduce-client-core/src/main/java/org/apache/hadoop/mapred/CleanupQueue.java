...
class CleanupQueue {
  ...
  public static final /* LogLOG=LogFactory.getLog(CleanupQueue.class); */
  ...
  protected static boolean deletePath(PathDeletionContext context)
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.trying_delete(String.valueOf(context.fullPath)).tag("methodCall").debug();
    }
  ...
  private static class PathCleanupThread extends Thread {
    ...
    public void run() {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.started(getName()).tag("methodCall").debug();
      }
      ...
      while (true) {
        try {
          ...
          if (!deletePath(context)) {
            ...
            LOG.cleanupthread_unable_delete_path(String.valueOf(context.fullPath)).tag("methodCall").warn();
          }
          else if (LogGlobal.isDebugEnabled()) {
            /* LOG.debug("DELETED "+context.fullPath) */
            LOG.deleted(String.valueOf(context.fullPath)).tag("methodCall").debug();
          }
        } catch (InterruptedException t) {
          /* LOG.warn("Interrupted deletion of "+context.fullPath) */
          LOG.interrupted_deletion(String.valueOf(context.fullPath)).tag("methodCall").warn();
          return;
        } catch (Exception e) {
          /* LOG.warn("Error deleting path "+context.fullPath+": "+e) */
          LOG.error_deleting_path(String.valueOf(context.fullPath), e.toString()).tag("methodCall").warn();
        } 
      }
    }
  }
