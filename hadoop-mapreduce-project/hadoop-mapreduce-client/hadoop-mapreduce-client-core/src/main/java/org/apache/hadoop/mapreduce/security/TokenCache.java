...
public class TokenCache {
  ...
  private static final /* LogLOG=LogFactory.getLog(TokenCache.class); */
  ...
  static void obtainTokensForNamenodesInternal(FileSystem fs, 
    ...
    if (tokens != null) {
      for (Token<?> token : tokens) {
        ...
        LOG.got_for(String.valueOf(fs.getUri()), token.toString()).tag("methodCall").info();
      }
    }
  }
  ...
  throws IOException {
    ...
    if(LogGlobal.isDebugEnabled()) {
      ...
      LOG.task_loaded_jobtokenfile_from_num_sec_nu(String.valueOf(localJobTokenFile.toUri().getPath()), String.valueOf(ts.numberOfSecretKeys()), String.valueOf(ts.numberOfTokens())).tag("methodCall").debug();
    }
    return ts;
  }
