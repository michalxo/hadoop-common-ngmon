...
public class FixedLengthRecordReader
  ...
  private static final /* LogLOG=LogFactory.getLog(FixedLengthRecordReader.class); */
  ...
  public void initialize(Configuration job, long splitStart, long splitLength,
    ...
    if (null != codec) {
      ...
      LOG.compressed_input_cannot_compute_number_r().info();
    } else {
      fileIn.seek(start);
      filePosition = fileIn;
      inputStream = fileIn;
      ...
      LOG.expecting_records_each_with_length_bytes(numRecordsRemainingInSplit, recordLength, splitSize).info();
    }
    if (numBytesToSkip != 0) {
