...
class JobSubmitter {
  protected static final /* LogLOG=LogFactory.getLog(JobSubmitter.class); */
  ...
  private void copyAndConfigureFiles(Job job, Path submitJobDir,
    ...
    if (!(conf.getBoolean(Job.USED_GENERIC_PARSER, false))) {
      ...
      LOG.hadoop_command_line_option_parsing_not_i().warn();
    }
    ...
    LOG.default_filesystem(String.valueOf(jtFs.getUri())).tag("methodCall").debug();
    ...
    if (jobJar != null) {   // copy jar to JobTracker's fs
      ...
      LOG.job_jar_file_set_user_classes_see().warn();
    }
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    try {
      ...
      LOG.configuring_job_with_submit_dir(jobId.toString(), submitJobDir.toString()).debug();
      ...
      LOG.creating_splits(String.valueOf(jtFs.makeQualified(submitJobDir))).tag("methodCall").debug();
      int maps = writeSplits(job, submitJobDir);
      conf.setInt(MRJobConfig.NUM_MAPS, maps);
      /* LOG.info("number of splits:"+maps) */
      LOG.number_splits(maps).info();
      ...
      if (status == null) {
        ...
        LOG.cleaning_staging_area(submitJobDir.toString()).info();
  ...
  private void printTokens(JobID jobId,
    ...
    LOG.submitting_tokens_for_job(jobId.toString()).info();
  ...
  throws IOException {
    ...
    if(tokensFileName != null) {
      ...
      LOG.loading_users_secret_keys_from(tokensFileName).info();
      ...
      if(json_error)
        ...
        LOG.couldnt_parse_token_cache_json_file().warn();
    }
  }
  ...
  throws IOException{
    ...
    LOG.adding_following_namenodes_delegation_to(String.valueOf(Arrays.toString(nameNodes))).tag("methodCall").debug();
    if(nameNodes != null) {
