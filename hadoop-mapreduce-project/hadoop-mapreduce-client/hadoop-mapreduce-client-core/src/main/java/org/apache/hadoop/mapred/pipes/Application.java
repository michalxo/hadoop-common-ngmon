...
class Application<K1 extends WritableComparable, V1 extends Writable,
  ...
  private static final /* LogLOG=LogFactory.getLog(Application.class.getName()); */
  ...
  Application(JobConf conf, 
    ...
    LOG.authentication_succeeded().debug();
    downlink.start();
  ...
  void waitForAuthentication() throws IOException,
    ...
    LOG.waiting_for_authentication_response().debug();
  ...
  void abort(Throwable t) throws IOException {
    ...
    LOG.aborting_because(StringUtils.stringifyException(t)).tag("methodCall").info();
    try {
      downlink.abort();
      downlink.flush();
    } catch (IOException e) {
