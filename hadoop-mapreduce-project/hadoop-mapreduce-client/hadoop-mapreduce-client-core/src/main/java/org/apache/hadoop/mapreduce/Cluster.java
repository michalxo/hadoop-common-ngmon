...
public class Cluster {
  ...
  private static final /* LogLOG=LogFactory.getLog(Cluster.class); */
  ...
  private void initialize(InetSocketAddress jobTrackAddr, Configuration conf)
    ...
    synchronized (frameworkLoader) {
      for (ClientProtocolProvider provider : frameworkLoader) {
        ...
        LOG.trying_clientprotocolprovider(String.valueOf(provider.getClass().getName())).tag("methodCall").debug();
        ...
        try {
          ...
          if (clientProtocol != null) {
            ...
            LOG.picked_clientprotocolprovider(String.valueOf(provider.getClass().getName())).tag("methodCall").debug();
            break;
          }
          else {
            /* LOG.debug("Cannot pick "+provider.getClass().getName()+" as the ClientProtocolProvider - returned null protocol") */
            LOG.cannot_pick_clientprotocolprovider_retur(String.valueOf(provider.getClass().getName())).tag("methodCall").debug();
          }
        } 
        catch (Exception e) {
          /* LOG.info("Failed to use "+provider.getClass().getName()+" due to error: "+e.getMessage()) */
          LOG.failed_use_due_error(String.valueOf(provider.getClass().getName()), String.valueOf(e.getMessage())).tag("methodCall").info();
        }
      }
    }
