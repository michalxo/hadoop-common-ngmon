...
public class FileOutputCommitter extends OutputCommitter {
  private static final /* LogLOG=LogFactory.getLog(FileOutputCommitter.class); */
  ...
  public void setupJob(JobContext context) throws IOException {
    if (hasOutputPath()) {
      ...
      if (!fs.mkdirs(jobAttemptPath)) {
        ...
        LOG.mkdirs_failed_create(jobAttemptPath.toString()).error();
      }
    } else {
      /* LOG.warn("Output Path is null in setupJob()") */
      LOG.output_path_null_setupjob().warn();
    }
  }
  ...
  public void commitJob(JobContext context) throws IOException {
    if (hasOutputPath()) {
      ...
      LOG.output_path_null_commitjob().warn();
    }
  }
  ...
  private static void mergePaths(FileSystem fs, final FileStatus from,
    ...
    throws IOException {
     ...
     LOG.merging_data_from(from.toString(), to.toString()).debug();
     if(from.isFile()) {
       if(fs.exists(to)) {
  ...
  public void cleanupJob(JobContext context) throws IOException {
    if (hasOutputPath()) {
      ...
      LOG.output_path_null_cleanupjob().warn();
    }
  }
  ...
  throws IOException {
    ...
    if (hasOutputPath()) {
      ...
      if (fs.exists(taskAttemptPath)) {
        ...
        LOG.saved_output_task(attemptId.toString(), committedTaskPath.toString()).info();
      } else {
        /* LOG.warn("No Output found for "+attemptId) */
        LOG.output_found_for(attemptId.toString()).warn();
      }
    } else {
      /* LOG.warn("Output Path is null in commitTask()") */
      LOG.output_path_null_committask().warn();
    }
  }
  ...
  public void abortTask(TaskAttemptContext context, Path taskAttemptPath) throws IOException {
    if (hasOutputPath()) { 
      ...
      if(!fs.delete(taskAttemptPath, true)) {
        ...
        LOG.could_not_delete(taskAttemptPath.toString()).warn();
      }
    } else {
      /* LOG.warn("Output Path is null in abortTask()") */
      LOG.output_path_null_aborttask().warn();
    }
  }
  ...
  public void recoverTask(TaskAttemptContext context)
    ...
    if(hasOutputPath()) {
      ...
      LOG.trying_recover_task_from_into(previousCommittedTaskPath.toString(), committedTaskPath.toString()).debug();
      if (fs.exists(previousCommittedTaskPath)) {
        ...
        LOG.saved_output(attemptId.toString(), committedTaskPath.toString()).info();
      } else {
        /* LOG.warn(attemptId+" had no output to recover.") */
        LOG.had_output_recover(attemptId.toString()).warn();
      }
    } else {
      /* LOG.warn("Output Path is null in recoverTask()") */
      LOG.output_path_null_recovertask().warn();
    }
  }
}
