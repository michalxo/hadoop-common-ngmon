...
class IndexCache {
  ...
  private static final /* LogLOG=LogFactory.getLog(IndexCache.class); */
  ...
  public IndexCache(JobConf conf) {
    ...
    LOG.indexcache_created_with_max_memory(totalMemoryAllowed).info();
  }
  ...
  public IndexRecord getIndexInformation(String mapId, int reduce,
    ...
    if (info == null) {
      ...
      LOG.indexcache_hit_mapid_found(mapId).debug();
    }
  ...
  private IndexInformation readIndexFileToCache(Path indexFileName,
    ...
    LOG.indexcache_miss_mapid_not_found(mapId).debug() ;
  ...
  public void removeMap(String mapId) {
    ...
    if (info != null) {
      ...
      if (!queue.remove(mapId)) {
        ...
        LOG.map_not_found_queue(mapId).warn();
      }
    } else {
      /* LOG.info("Map ID "+mapId+" not found in cache") */
      LOG.map_not_found_cache(mapId).info();
    }
  }
