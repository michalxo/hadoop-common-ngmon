...
class DeprecatedQueueConfigurationParser extends QueueConfigurationParser {
  private static final /* LogLOG=LogFactory.getLog(DeprecatedQueueConfigurationParser.class); */
  ...
  private List<Queue> createQueues(Configuration conf) {
    ...
    for (String name : queueNameValues) {
      try {
        ...
        LOG.not_able_initialize_queue(name).warn();
      }
    }
    return list;
  }
  ...
  private boolean deprecatedConf(Configuration conf) {
    ...
    if (queueNameValues == null) {
      ...
      LOG.configuring_mapred_site_xml_hadoop_site_(MAPRED_QUEUE_NAMES_KEY, QUEUE_CONF_FILE_NAME, QUEUE_CONF_FILE_NAME).warn();
      ...
      for (String queue : queues) {
        for (QueueACL qAcl : QueueACL.values()) {
          ...
          if (aclString != null) {
            ...
            LOG.configuring_queue_acls_mapred_site_xml_h(QUEUE_CONF_FILE_NAME).warn();
