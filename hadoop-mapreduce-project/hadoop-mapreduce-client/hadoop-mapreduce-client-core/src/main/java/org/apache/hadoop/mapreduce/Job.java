...
public class Job extends JobContextImpl implements JobContext {  
  private static final /* LogLOG=LogFactory.getLog(Job.class); */
  ...
  public void submit() 
    ...
    LOG.url_track_job(getTrackingURL()).tag("methodCall").info();
   }
  ...
  public boolean monitorAndPrintJob() 
    ...
    LOG.running_job(jobId.toString()).info();
    int eventCounter = 0;
    ...
    while (!isComplete() || !reportedAfterCompletion) {
      ...
      if (!reportedUberMode) {
        ...
        LOG.job_running_uber_mode(jobId.toString(), isUber()).tag("methodCall").info();
      }      
      String report = 
    ...
    if (success) {
      ...
      LOG.job_completed_successfully(jobId.toString()).info();
    } else {
      /* LOG.info("Job "+jobId+" failed with state "+status.getState()+" due to: "+status.getFailureInfo()) */
      LOG.job_failed_with_state_due(jobId.toString(), String.valueOf(status.getState()), String.valueOf(status.getFailureInfo())).tag("methodCall").info();
    }
    Counters counters = getCounters();
    if (counters != null) {
      /* LOG.info(counters.toString()) */
      LOG.countersmethodca(String.valueOf(counters.toString())).tag("methodCall").info();
    }
    return success;
  }
    ...
    for (TaskCompletionEvent event : events) {
      ...
      case SUCCEEDED:
        if (event.getStatus() == 
          ...
          LOG.eventmethodcall(String.valueOf(event.toString())).tag("methodCall").info();
        }
        break; 
      case FAILED:
        if (event.getStatus() == 
  ...
  public static int getProgressPollInterval(Configuration conf) {
    ...
    if (progMonitorPollIntervalMillis < 1) {
      ...
      LOG.has_been_set_invalid_value_replacing(PROGRESS_MONITOR_POLL_INTERVAL_KEY, DEFAULT_MONITOR_POLL_INTERVAL).warn();
  ...
  public static int getCompletionPollInterval(Configuration conf) {
    ...
    if (completionPollIntervalMillis < 1) { 
      ...
      LOG.has_been_set_invalid_value_replacing(COMPLETION_POLL_INTERVAL_KEY, DEFAULT_COMPLETION_POLL_INTERVAL).warn();
