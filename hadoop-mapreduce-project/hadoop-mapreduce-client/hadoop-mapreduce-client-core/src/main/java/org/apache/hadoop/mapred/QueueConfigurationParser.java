...
class QueueConfigurationParser {
  private static final /* LogLOG=LogFactory.getLog(QueueConfigurationParser.class); */
  ...
  protected Queue loadResource(InputStream resourceInput)
    ...
    try {
      ...
      LOG.failed_set_setxincludeaware_true_for_par(docBuilderFactory.toString(), NAME_SEPARATOR, e.toString()).info();
    }
  ...
  private Queue parseResource(Element queuesNode) {
    ...
    try {
      if (!QUEUES_TAG.equals(queuesNode.getTagName())) {
        ...
        LOG.bad_conf_file_top_level_element().info();
      ...
      if (acls != null) {
        ...
        LOG.configuring_flag_not_valid_this_tag_mapr(ACLS_ENABLED_TAG, String.valueOf(QueueManager.QUEUE_CONF_FILE_NAME), String.valueOf(MRConfig.MR_ACLS_ENABLED), String.valueOf(MRConfig.MR_ACLS_ENABLED)).tag("methodCall").warn();
      }

      NodeList props = queuesNode.getChildNodes();
      if (props == null || props.getLength() <= 0) {
        /* LOG.info(" Bad configuration no queues defined ") */
        LOG.bad_configuration_queues_defined().info();
        throw new RuntimeException(" No queues defined ");
      }
      ...
      for (int i = 0; i < props.getLength(); i++) {
        ...
        if (!propNode.getNodeName().equals(QUEUE_TAG)) {
          ...
          LOG.root_level_only_queue_tags_are().info();
          throw
      ...
      LOG.error_parsing_conf_file(e.toString()).info();
      throw new RuntimeException(e);
    }
  }
