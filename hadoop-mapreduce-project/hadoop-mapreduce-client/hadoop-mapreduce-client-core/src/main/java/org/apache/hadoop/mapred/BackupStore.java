...
public class BackupStore<K,V> {
  ...
  private static final /* LogLOG=LogFactory.getLog(BackupStore.class.getName()); */
  ...
  throws IOException {
    ...
    LOG.created_new_backupstore_with_memory(maxSize).info();
  ...
  public void mark() throws IOException {
    ...
    LOG.setting_firssegmentoffset(currentKVOffset).debug();
  }
  ...
  public void reset() throws IOException {
    ...
    LOG.reset_first_segment_offset_segment_list(firstSegmentOffset, String.valueOf(segmentList.size())).tag("methodCall").debug();
  }
  ...
  class MemoryCache {
    ...
    void reinitialize(boolean clearAll) {
      ...
      LOG.created_new_mem_block(allocatedSize).debug();
    }
    ...
    throws IOException {
      ...
      LOG.write_mem(String.valueOf(segmentList.size())).tag("methodCall").debug();
    }
    ...
    void createInMemorySegment () throws IOException {
      ...
      LOG.added_memory_segment_list_list_size(String.valueOf(segmentList.size())).tag("methodCall").debug();
    }
  }
  ...
  class FileCache {
    ...
    throws IOException {
      ...
      LOG.write_disk(String.valueOf(segmentList.size())).tag("methodCall").debug();
    }
    ...
    void createInDiskSegment() throws IOException {
      ...
      LOG.disk_segment_added_list_size(String.valueOf(segmentList.size())).tag("methodCall").debug();
    }
    ...
    private Writer<K,V> createSpillFile() throws IOException {
      ...
      LOG.created_file(tmp.toString()).info();
  ...
  static class BackupRamManager implements RamManager {
    ...
    public boolean reserve(int requestedSize, InputStream in) {
      ...
      LOG.reserve_int_inputstream_not_supported_ba().warn();
      return false;
    }
    ...
    int reserve(int requestedSize) {
      ...
      LOG.reserving_requested(reservedSize, requestedSize).debug();
      return reservedSize;
    }
    ...
    int reserve(int requestedSize, int minSize) {
      if (availableSize < minSize) {
        ...
        LOG.space_available_available_minsize(availableSize, minSize).debug();
        return 0;
      } else {
    ...
    public void unreserve(int requestedSize) {
      ...
      LOG.unreserving_available(requestedSize, availableSize).debug();
    }
