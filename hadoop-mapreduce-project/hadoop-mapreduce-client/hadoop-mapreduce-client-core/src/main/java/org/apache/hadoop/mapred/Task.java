...
abstract public class Task implements Writable, Configurable {
  private static final /* LogLOG=LogFactory.getLog(Task.class); */
  ...
  protected void reportFatalError(TaskAttemptID id, Throwable throwable, 
    ...
    try {
      ...
      LOG.failed_contact_tasktracker(ioe.toString()).fatal();
      System.exit(-1);
    }
  }
  ...
  public void initialize(JobConf job, JobID id, 
    ...
    if (useNewApi) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.using_new_api_for_output_committer().debug();
      }
      outputFormat =
    ...
    LOG.using_resourcecalculatorprocesstree(pTree.toString()).info();
    if (pTree != null) {
  ...
  public static String normalizeStatus(String status, Configuration conf) {
    ...
    if (status.length() > progressStatusLength) {
      ...
      LOG.task_status_truncated_max_limit_characte(status, progressStatusLength).warn();
      ...
      while (!taskDone.get()) {
        ...
        try {
          ...
          if (!taskFound) {
            ...
            LOG.parent_died_exiting(taskId.toString()).warn();
            resetDoneFlag();
            System.exit(66);
          }
          ...
          if (lastPreempt ^ mustPreempt.get()) {
            ...
            LOG.preemption_task_setting_mustpreempt_give(String.valueOf(mustPreempt.get()), String.valueOf(amFeedback.getPreemption()), taskId.toString(), String.valueOf(taskStatus.getPhase())).tag("methodCall").info();
          }
          ...
          LOG.communication_exception(StringUtils.stringifyException(t)).tag("methodCall").info();
          remainingRetries -=1;
          if (remainingRetries == 0) {
            ReflectionUtils.logThreadInfo(LogFactory.getLog(Task.class), "Communication exception", 0);
            /* LOG.warn("Last retry, killing "+taskId) */
            LOG.last_retry_killing(taskId.toString()).warn();
            resetDoneFlag();
            System.exit(65);
          }
        }
  ...
  protected void reportNextRecordRange(final TaskUmbilicalProtocol umbilical, 
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.sending_reportnextrecordrange(range.toString()).debug();
    }
  ...
  public void done(TaskUmbilicalProtocol umbilical,
    ...
    LOG.task_done_and_process_committing(taskId.toString()).info();
    ...
    if (commitRequired) {
      ...
      while (true) {
        try {
          ...
          LOG.failure_sending_commit_pending(StringUtils.stringifyException(ie)).tag("methodCall").warn();
          if (--retries == 0) {
            System.exit(67);
          }
        }
  ...
  throws IOException {
    ...
    while (true) {
      try {
        ...
        LOG.failure_sending_status_update(StringUtils.stringifyException(ie)).tag("methodCall").warn();
        if (--retries == 0) {
          throw ie;
        }
      }
   ...
   private long calculateOutputSize() throws IOException {
    ...
    if (isMapTask() && conf.getNumReduceTasks() > 0) {
      try {
        ...
        LOG.could_not_find_output_size(e.toString()).warn();
      }
    }
    return -1;
  }
  ...
  private void sendDone(TaskUmbilicalProtocol umbilical) throws IOException {
    ...
    while (true) {
      try {
        ...
        LOG.task_done(taskId.toString()).info();
        return;
      } catch (IOException ie) {
        /* LOG.warn("Failure signalling completion: "+StringUtils.stringifyException(ie)) */
        LOG.failure_signalling_completion(StringUtils.stringifyException(ie)).tag("methodCall").warn();
        if (--retries == 0) {
          throw ie;
        }
      }
  ...
  private void commit(TaskUmbilicalProtocol umbilical,
    ...
    while (true) {
      try {
        ...
        LOG.failure_asking_whether_task_can_commit(StringUtils.stringifyException(ie)).tag("methodCall").warn();
        if (--retries == 0) {
    ...
    try {
      ...
      LOG.task_allowed_commit_now(taskId.toString()).info();
      committer.commitTask(taskContext);
      return;
    } catch (IOException iee) {
      /* LOG.warn("Failure committing: "+StringUtils.stringifyException(iee)) */
      LOG.failure_committing(StringUtils.stringifyException(iee)).tag("methodCall").warn();
  ...
  void discardOutput(TaskAttemptContext taskContext) {
    try {
      ...
      LOG.failure_cleaning(StringUtils.stringifyException(ioe)).tag("methodCall").warn();
    }
  }
  ...
  throws IOException {
    ...
    LOG.runnning_cleanup_for_task().info();
    // do the cleanup
    committer.abortTask(taskContext);
  }
  ...
  protected void runJobCleanupTask(TaskUmbilicalProtocol umbilical,
    ...
    LOG.cleaning_job().info();
    if (jobRunStateForCleanup == JobStatus.State.FAILED 
        || jobRunStateForCleanup == JobStatus.State.KILLED) {
      /* LOG.info("Aborting job with runstate : "+jobRunStateForCleanup.name()) */
      LOG.aborting_job_with_runstate(String.valueOf(jobRunStateForCleanup.name())).tag("methodCall").info();
