...
class Fetcher<K,V> extends Thread {
  ...
  private static final /* LogLOG=LogFactory.getLog(Fetcher.class); */
  ...
  public void shutDown() throws InterruptedException {
    ...
    try {
      ...
      LOG.got_interrupt_while_joining(getName(), ie.toString()).tag("methodCall").warn();
    }
    if (sslFactory != null) {
      sslFactory.destroy();
    }
  ...
  protected void copyFromHost(MapHost host) throws IOException {
    ...
    if(LogGlobal.isDebugEnabled()) {
      ...
      LOG.fetcher_going_fetch_from_for(id, host.toString(), maps.toString()).debug();
    }
    ...
    try {
      ...
      LOG.url_enchash_replyhash(msgToEncode, encHash, replyHash).debug();
      // verify that replyHash is HMac of encHash
      SecureShuffleUtils.verifyReply(replyHash, encHash, shuffleSecretKey);
      /* LOG.info("for url="+msgToEncode+" sent hash and received reply") */
      LOG.for_url_sent_hash_and_received(msgToEncode).info();
    } catch (IOException ie) {
      boolean connectExcpt = ie instanceof ConnectException;
      ioErrs.increment(1);
      /* LOG.warn("Failed to connect to "+host+" with "+remaining.size()+" map outputs",ie) */
      LOG.failed_connect_with_map_outputs(host.toString(), String.valueOf(remaining.size()), ie.toString()).tag("methodCall").warn();
    ...
    try {
      ...
      if(failedTasks != null && failedTasks.length > 0) {
        ...
        LOG.copymapoutput_failed_for_tasks(String.valueOf(Arrays.toString(failedTasks))).tag("methodCall").warn();
  ...
  private TaskAttemptID[] copyMapOutput(MapHost host,
    ...
    try {
      ...
      try {
        ...
        LOG.invalid_map(e.toString()).warn();
      ...
      if(LogGlobal.isDebugEnabled()) {
        ...
        LOG.header_len_decomp_len(mapId.toString(), compressedLength, decompressedLength).debug();
      }
      ...
      if (mapOutput == null) {
        ...
        LOG.fetcher_mergemanager_returned_status_wai(id).info();
      ...
      try {
        ...
        LOG.fetcher_about_shuffle_output_map_decomp_(id, String.valueOf(mapOutput.getMapId()), decompressedLength, compressedLength, String.valueOf(mapOutput.getDescription())).tag("methodCall").info();
        mapOutput.shuffle(host, input, compressedLength, decompressedLength,
            metrics, reporter);
      } catch (java.lang.InternalError e) {
        /* LOG.warn("Failed to shuffle for fetcher#"+id,e) */
        LOG.failed_shuffle_for_fetcher(id, e.toString()).warn();
      ...
      if (mapId == null || mapOutput == null) {
        ...
        LOG.fetcher_failed_read_map_header_decomp(id, mapId.toString(), decompressedLength, compressedLength, ioe.toString()).info();
        if(mapId == null) {
          return remaining.toArray(new TaskAttemptID[remaining.size()]);
        } else {
          return new TaskAttemptID[] {mapId};
      ...
      LOG.failed_shuffle_output_from(mapId.toString(), String.valueOf(host.getHostName()), ioe.toString()).tag("methodCall").warn(); 
  ...
  private boolean verifySanity(long compressedLength, long decompressedLength,
    ...
    if (compressedLength < 0 || decompressedLength < 0) {
      ...
      LOG.invalid_lengths_map_output_header_len_de(getName(), mapId.toString(), compressedLength, decompressedLength).tag("methodCall").warn();
      return false;
    }
    ...
    if (forReduce != reduce) {
      ...
      LOG.data_for_wrong_reduce_map_len_decomp_for(getName(), mapId.toString(), compressedLength, decompressedLength, forReduce).tag("methodCall").warn();
      return false;
    }
    ...
    if (!remaining.contains(mapId)) {
      ...
      LOG.invalid_map_output_received_output_for(mapId.toString()).warn();
      return false;
    }
  ...
  private URL getMapOutputURL(MapHost host, List<TaskAttemptID> maps
    ...
    LOG.mapoutput_url_for(host.toString(), String.valueOf(url.toString())).tag("methodCall").debug();
