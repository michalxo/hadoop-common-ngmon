...
public class MapTask extends Task {
  ...
  private static final /* LogLOG=LogFactory.getLog(MapTask.class.getName()); */
  ...
  class SkippingRecordReader<K, V> extends TrackedRecordReader<K,V> {
    ...
    throws IOException {
      if(!skipIt.hasNext()) {
        ...
        LOG.further_records_got_skipped().warn();
        return false;
      }
      boolean ret = moveToNext(key, value);
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  private <KEY, VALUE> MapOutputCollector<KEY, VALUE>
    ...
    LOG.map_output_collector_class(String.valueOf(collector.getClass().getName())).tag("methodCall").info();
  ...
  void runOldMapper(final JobConf job,
    ...
    LOG.numreducetasks(numReduceTasks).info();
  ...
  private void updateJobWithSplit(final JobConf job, InputSplit inputSplit) {
    ...
    LOG.processing_split(inputSplit.toString()).info();
  }
  ...
  void runNewMapper(final JobConf job,
    ...
    LOG.processing_split(split.toString()).info();
  ...
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    public void init(MapOutputCollector.Context context
      ...
      LOG.jobcontextio_sor(String.valueOf(JobContext.IO_SORT_MB), sortmb).tag("methodCall").info();
      /* LOG.info("soft limit at "+softLimit) */
      LOG.soft_limit(softLimit).info();
      /* LOG.info("bufstart = "+bufstart+"; bufvoid = "+bufvoid) */
      LOG.bufstart_bufvoid(bufstart, bufvoid).info();
      /* LOG.info("kvstart = "+kvstart+"; length = "+maxRec) */
      LOG.kvstart_length(kvstart, maxRec).info();
    ...
    public synchronized void collect(K key, V value, final int partition
      ...
      try {
        ...
        LOG.record_too_large_for_memory_buffer(String.valueOf(e.getMessage())).tag("methodCall").info();
    ...
    private void setEquator(int pos) {
      ...
      LOG.equator_kvi(pos, kvindex, (kvindex*4)).tag("mathExp").info();
    }
    ...
    private void resetSpill() {
      ...
      kvstart = kvend = (int)
        (((long)aligned - METASIZE + kvbuffer.length) % kvbuffer.length) / 4;
      /* LOG.info("(RESET) equator "+e+" kv "+kvstart+"("+(kvstart*4)+")"+" kvi "+kvindex+"("+(kvindex*4)+")") */
      LOG.reset_equator_kvi(e, kvstart, (kvstart*4), kvindex, (kvindex*4)).tag("mathExp").info();
    }
    ...
    public void flush() throws IOException, ClassNotFoundException,
      ...
      LOG.starting_flush_map_output().info();
      spillLock.lock();
      try {
        while (spillInProgress) {
          reporter.progress();
        ...
        if (kvindex != kvend) {
          ...
          LOG.spilling_map_output().info();
          /* LOG.info("bufstart = "+bufstart+"; bufend = "+bufmark+"; bufvoid = "+bufvoid) */
          LOG.bufstart_bufend_bufvoid(bufstart, bufmark, bufvoid).info();
          /* LOG.info("kvstart = "+kvstart+"("+(kvstart*4)+"); kvend = "+kvend+"("+(kvend*4)+"); length = "+(distanceTo(kvend,kvstart,kvmeta.capacity())+1)+"/"+maxRec) */
          LOG.kvstart_kvend_length(kvstart, (kvstart*4), kvend, (kvend*4), (distanceTo(kvend,kvstart,kvmeta.capacity())+1), maxRec).tag("mathExp").info();
          sortAndSpill();
        }
      } catch (InterruptedException e) {
    ...
    private void sortAndSpill() throws IOException, ClassNotFoundException,
      ...
      try {
        ...
        LOG.finished_spill(numSpills).info();
        ++numSpills;
      } finally {
        ...
        for (int parts = 0; parts < partitions; parts++) {
          ...
          for(int i = 0; i < numSpills; i++) {
            ...
            if (LogGlobal.isDebugEnabled()) {
              ...
              LOG.mapid_reducer_spill(mapId.toString(), parts, i, String.valueOf(indexRecord.startOffset), String.valueOf(indexRecord.rawLength), String.valueOf(indexRecord.partLength)).tag("methodCall").debug();
            }
          }
  ...
  void closeQuietly(RecordReader<INKEY, INVALUE> c) {
    if (c != null) {
      try {
        ...
        LOG.ignoring_exception_during_close_for(c.toString(), ie.toString()).info();
      }
    }
  }
