...
public abstract class TaskStatus implements Writable, Cloneable {
  static final /* LogLOG=LogFactory.getLog(TaskStatus.class.getName()); */
  ...
  public void setDiagnosticInfo(String info) {
    ...
    if (diagnosticInfo != null 
      ...
      LOG.task_diagnostic_info_for_task(taskid.toString(), info).info();
      return;
    }
    diagnosticInfo = 
    ...
    if (diagnosticInfo != null 
      ...
      LOG.task_diagnostic_info_for_task(taskid.toString(), diagnosticInfo).info();
  ...
  public void setStateString(String stateString) {
    if (stateString != null) {
      if (stateString.length() <= getMaxStringSize()) {
        ...
        LOG.state_string_for_task(taskid.toString(), stateString).info();
  ...
  void setFinishTime(long finishTime) {
    if(this.getStartTime() > 0 && finishTime > 0) {
      ...
      /* LOG.error("Trying to set finish time for task "+taskid+" when no start time is set, stackTrace is : "+StringUtils.stringifyException(newException())) */
      LOG.trying_set_finish_time_for_task_when(taskid.toString(), StringUtils.stringifyException(new Exception())).tag("methodCall").error();
    }
  }
  /**
  ...
  void setStartTime(long startTime) {
    ...
    if (startTime > 0) {
      ...
      //Using String utils to get the stack trace.
      /* LOG.error("Trying to set illegal startTime for task : "+taskid+".Stack trace is : "+StringUtils.stringifyException(newException())) */
      LOG.trying_set_illegal_starttime_for_task_st(taskid.toString(), StringUtils.stringifyException(new Exception())).tag("methodCall").error();
    }
  }
  /**
