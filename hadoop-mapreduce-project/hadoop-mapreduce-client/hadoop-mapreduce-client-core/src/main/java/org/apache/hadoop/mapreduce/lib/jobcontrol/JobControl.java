...
public class JobControl implements Runnable {
  private static final /* LogLOG=LogFactory.getLog(JobControl.class); */
  ...
  public void run() {
    ...
    try {
      ...
      while (true) {
        ...
        synchronized(this) {
          ...
          while(it.hasNext()) {
            ...
            LOG.checking_state_job(j.toString()).debug();
            switch(j.checkState()) {
            case SUCCESS:
      ...
      LOG.error_while_trying_run_jobs(t.toString()).error();
  ...
  synchronized private void failAllJobs(Throwable t) {
    ...
    while(it.hasNext()) {
      ...
      try {
        ...
        LOG.error_while_tyring_clean(String.valueOf(j.getJobName()), e.toString()).tag("methodCall").error();
 ...
 LOG.job_control_has_circular_dependency_for(String.valueOf(controlledJob.getJobName())).tag("methodCall").error();
	break;
      }
    }
    return cyclePresent;
