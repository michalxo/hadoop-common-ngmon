...
class EventFetcher<K,V> extends Thread {
  ...
  private static final /* LogLOG=LogFactory.getLog(EventFetcher.class); */
  ...
  public void run() {
    ...
    LOG.thread_started(reduce.toString(), getName()).tag("methodCall").info();
      ...
      while (!stopped && !Thread.currentThread().isInterrupted()) {
        try {
          ...
          if (numNewMaps > 0) {
            ...
            LOG.got_new_map_outputs(reduce.toString(), numNewMaps).info();
          }
          /* LOG.debug("GetMapEventsThread about to sleep for "+SLEEP_TIME) */
          LOG.getmapeventsthread_about_sleep_for(SLEEP_TIME).debug();
          if (!Thread.currentThread().isInterrupted()) {
            Thread.sleep(SLEEP_TIME);
          }
        } catch (InterruptedException e) {
          /* LOG.info("EventFetcher is interrupted.. Returning") */
          LOG.eventfetcher_interrupted_returning().info();
          return;
        } catch (IOException ie) {
          /* LOG.info("Exception in getting events",ie) */
          LOG.exception_getting_events(ie.toString()).info();
  ...
  public void shutDown() {
    ...
    try {
      ...
      LOG.got_interrupted_while_joining(getName(), ie.toString()).tag("methodCall").warn();
    }
  }
  ...
  protected int getMapCompletionEvents()
    ...
    do {
      ...
      LOG.got_map_completion_events_from(String.valueOf(events.length), fromEventIdx).tag("methodCall").debug();
