...
public class CompletedJob implements org.apache.hadoop.mapreduce.v2.app.job.Job {
  ...
  static final /* LogLOG=LogFactory.getLog(CompletedJob.class); */
  ...
  public CompletedJob(Configuration conf, JobId jobId, Path historyFile, 
    ...
    LOG.loading_job_from_file(jobId.toString(), historyFile.toString()).info();
    this.conf = conf;
    this.jobId = jobId;
    this.user = userName;
    this.info = info;
  ...
  private void constructTaskAttemptCompletionEvents() {
    ...
    for (TaskAttempt taskAttempt : allTaskAttempts) {
      ...
      try {
        ...
        LOG.cannot_constuct_tacestatus_from_taskatem(taStateString, String.valueOf(taskAttempt.getID())).tag("methodCall").warn();
      }
  ...
  protected synchronized void loadFullHistoryData(boolean loadTasks,
    ...
    LOG.loading_history_file(historyFileAbsolute.toString()).info();
