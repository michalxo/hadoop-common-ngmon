...
public class HistoryServerFileSystemStateStoreService
  ...
  public static final /* LogLOG=LogFactory.getLog(HistoryServerFileSystemStateStoreService.class); */
  ...
  protected void initStorage(Configuration conf)
    ...
    LOG.using_for_history_server_state_storage(storeUri).info();
  ...
  public HistoryServerState loadState() throws IOException {
    ...
    LOG.loading_history_server_state_from(rootStatePath.toString()).info();
  ...
  public void removeToken(MRDelegationTokenIdentifier tokenId)
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.removing_token(String.valueOf(tokenId.getSequenceNumber())).tag("methodCall").debug();
    }
  ...
  public void storeTokenMasterKey(DelegationKey key) throws IOException {
    ...
    try {
      ...
      IOUtils.cleanup(null, dataStream);
    }
  ...
  private int loadTokensFromBucket(HistoryServerState state, Path bucket)
    ...
    for (FileStatus stat : tokenStats) {
      ...
      if (name.startsWith(TOKEN_FILE_PREFIX)) {
        ...
        LOG.skipping_unexpected_file_history_server_(String.valueOf(stat.getPath())).tag("methodCall").warn();
      }
    }
    return numTokens;
  }
  ...
  private void loadTokenState(HistoryServerState state) throws IOException {
    ...
    LOG.loaded_master_keys_and_tokens_from(numKeys, numTokens, tokenStatePath.toString()).info();
  }
}
