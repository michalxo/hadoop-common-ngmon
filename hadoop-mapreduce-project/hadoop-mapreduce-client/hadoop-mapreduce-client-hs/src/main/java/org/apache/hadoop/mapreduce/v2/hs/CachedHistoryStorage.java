...
public class CachedHistoryStorage extends AbstractService implements
    HistoryStorage {
  private static final /* LogLOG=LogFactory.getLog(CachedHistoryStorage.class); */
  ...
  public void serviceInit(Configuration conf) throws Exception {
    ...
    LOG.cachedhistorystorage_init().info();
  ...
  public void refreshLoadedJobCache() {
    if (getServiceState() == STATE.STARTED) {
      ...
      LOG.failed_execute_refreshloadedjobcache_cac().warn();
    }
  }
  ...
  private Job loadJob(HistoryFileInfo fileInfo) {
    try {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.adding_loaded_job_cache(String.valueOf(job.getID())).tag("methodCall").debug();
      }
  ...
  public Job getFullJob(JobId jobId) {
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.looking_for_job(jobId.toString()).debug();
    }
    try {
  ...
  public Map<JobId, Job> getAllPartialJobs() {
    ...
    LOG.called_getallpartialjobs().debug();
    ...
    try {
      ...
      LOG.error_trying_scan_for_all_fileinfos(e.toString()).warn();
      throw new YarnRuntimeException(e);
    }
    return result;
  }
