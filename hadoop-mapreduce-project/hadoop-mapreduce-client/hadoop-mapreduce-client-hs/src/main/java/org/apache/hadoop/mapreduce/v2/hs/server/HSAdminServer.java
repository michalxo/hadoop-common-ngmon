...
public class HSAdminServer extends AbstractService implements HSAdminProtocol {
  ...
  private static final /* LogLOG=LogFactory.getLog(HSAdminServer.class); */
  ...
  private UserGroupInformation checkAcls(String method) throws IOException {
    ...
    try {
      ...
      LOG.couldnt_get_current_user(ioe.toString()).warn();
    ...
    if (!adminAcl.isUserAllowed(user)) {
      ...
      LOG.user_doesnt_have_permission_call(String.valueOf(user.getShortUserName()), method).tag("methodCall").warn();
    ...
    LOG.admin_invoked_user(method, String.valueOf(user.getShortUserName())).tag("methodCall").info();
