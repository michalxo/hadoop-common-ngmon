...
public class JobHistory extends AbstractService implements HistoryContext {
  private static final /* LogLOG=LogFactory.getLog(JobHistory.class); */
  ...
  protected void serviceStop() throws Exception {
    ...
    LOG.stopping_jobhistory().info();
    if (scheduledExecutor != null) {
      /* LOG.info("Stopping History Cleaner/Move To Done") */
      LOG.stopping_history_cleaner_move_done().info();
      ...
      if (!scheduledExecutor.isShutdown()) {
        ...
        LOG.historycleanerservice_move_done_shutdown().warn();
  ...
  private class MoveIntermediateToDoneRunnable implements Runnable {
    ...
    public void run() {
      try {
        ...
        LOG.starting_scan_move_intermediate_done_fil().info();
        hsManager.scanIntermediateDirectory();
      } catch (IOException e) {
        /* LOG.error("Error while scanning intermediate done dir ",e) */
        LOG.error_while_scanning_intermediate_done_d(e.toString()).error();
      }
    }
  }
  ...
  private class HistoryCleaner implements Runnable {
    public void run() {
      ...
      LOG.history_cleaner_started().info();
      try {
        hsManager.clean();
      } catch (IOException e) {
        /* LOG.warn("Error trying to clean up ",e) */
        LOG.error_trying_clean(e.toString()).warn();
      }
      /* LOG.info("History Cleaner complete") */
      LOG.history_cleaner_complete().info();
    }
  }
  ...
  public Map<JobId, Job> getAllJobs(ApplicationId appID) {
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.called_getalljobs_appid(appID.toString()).debug();
    }
  ...
  public void refreshLoadedJobCache() {
    if (getServiceState() == STATE.STARTED) {
      ...
      LOG.failed_execute_refreshloadedjobcache_job().warn();
    }
  }
  ...
  public void refreshJobRetentionSettings() {
    if (getServiceState() == STATE.STARTED) {
      ...
      LOG.failed_execute_refreshjobretentionsettin().warn();
    }
  }
