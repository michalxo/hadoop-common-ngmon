...
public abstract class TaskAttemptImpl implements
  ...
  private static final /* LogLOG=LogFactory.getLog(TaskAttemptImpl.class); */
  ...
  private static ContainerLaunchContext createCommonContainerLaunchContext(
    ...
    try {
      ...
      if (jobJar != null) {
        ...
        LOG.job_jar_file_remote(String.valueOf(remoteJobJar.toUri().toASCIIString())).tag("methodCall").info();
      } else {
        // Job jar may be null. For e.g, for pipes, the job jar is the hadoop
        // mapreduce jar itself which is already on the classpath.
        /* LOG.info("Job jar is not present. "+"Not adding any jar to the list of resources.") */
        LOG.job_jar_not_present_not_adding().info();
      }
      // //////////// End of JobJar setup
      ...
      LOG.job_conf_file_remote(String.valueOf(remoteJobConfPath.toUri().toASCIIString())).tag("methodCall").info();
      ...
      LOG.adding_tokens_and_secret_keys_for(String.valueOf(credentials.numberOfTokens()), String.valueOf(credentials.numberOfSecretKeys())).tag("methodCall").info();
      ...
      LOG.size_containertokens_dob(String.valueOf(taskCredentials.numberOfTokens())).tag("methodCall").info();
      ...
      if (! shuffleProviders.isEmpty()) {
        ...
        for (final String shuffleProvider : shuffleProviders) {
          ...
          if (auxNames.contains(shuffleProvider)) {
                ...
                LOG.adding_shuffleprovider_service_serviceda(shuffleProvider).info();
  ...
  public void handle(TaskAttemptEvent event) {
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.processing_type(String.valueOf(event.getTaskAttemptID()), String.valueOf(event.getType())).tag("methodCall").debug();
    }
    writeLock.lock();
    try {
      ...
      try {
        ...
        LOG.cant_handle_this_event_current_state(String.valueOf(this.attemptId), e.toString()).tag("methodCall").error();
      ...
      if (oldState != getInternalState()) {
          ...
          LOG.taskattempt_transitioned_from(attemptId.toString(), oldState.toString(), getInternalState().toString()).tag("methodCall").info();
      }
    } finally {
      writeLock.unlock();
    }
  ...
  public TaskAttemptStateInternal recover(TaskAttemptInfo taInfo,
    ...
    if (recoverOutput
      ...
      try {
        ...
        LOG.recovered_output_from_task_attempt(attemptId.toString()).info();
      } catch (Exception e) {
        /* LOG.error("Unable to recover task attempt "+attemptId,e) */
        LOG.unable_recover_task_attempt(attemptId.toString(), e.toString()).error();
        /* LOG.info("Task attempt "+attemptId+" will be recovered as KILLED") */
        LOG.task_attempt_will_recovered_killed(attemptId.toString()).info();
    ...
    if (TaskAttemptState.SUCCEEDED.toString().equals(recoveredState)) {
      ...
      if (!TaskAttemptState.KILLED.toString().equals(recoveredState)) {
        if (String.valueOf(recoveredState).isEmpty()) {
          ...
          LOG.taskattempt_had_not_completed_recovering(attemptId.toString()).info();
        } else {
          /* LOG.warn("TaskAttempt "+attemptId+" found in unexpected state "+recoveredState+", recovering as KILLED") */
          LOG.taskattempt_found_unexpected_state_recov(attemptId.toString(), recoveredState).warn();
        }
    ...
    if (needToClean) {
      ...
      try {
        ...
        LOG.task_cleanup_failed_for_attempt(attemptId.toString(), e.toString()).warn();
      }
    }
  ...
  private void sendLaunchedEvents() {
    ...
    LOG.taskattempt_using_containerid(attemptId.toString(), String.valueOf(container.getId()), String.valueOf(StringInterner.weakIntern(container.getNodeId().toString()))).tag("methodCall").info();
  ...
  protected String resolveHost(String src) {
    ...
    try {
      ...
      LOG.failed_resolve_address_continuing_use_sa(src).warn();
    }
    return result;
  }
    ...
    public void transition(TaskAttemptImpl taskAttempt, 
      ...
      switch(finalState) {
        ...
        default:
          ...
          LOG.task_final_state_not_failed_killed(finalState.toString()).error();
      }
      if (taskAttempt.getLaunchTime() != 0) {
        ...
        LOG.not_generating_historyfinish_event_since(String.valueOf(taskAttempt.getID())).tag("methodCall").debug();
      }
    }
  }
  ...
  private static class KilledAfterSuccessTransition implements
    ...
    public TaskAttemptStateInternal transition(TaskAttemptImpl taskAttempt, 
      ...
      if(taskAttempt.getID().getTaskId().getTaskType() == TaskType.REDUCE) {
        ...
        LOG.ignoring_killed_event_for_successful_red(String.valueOf(taskAttempt.getID().toString())).tag("methodCall").info();
...
//      taskAttempt.logAttemptFinishedEvent(TaskAttemptStateInternal.KILLED); Not logging Map/Reduce attempts in case of failure.
  ...
  private static class DiagnosticInformationUpdater 
    ...
    public void transition(TaskAttemptImpl taskAttempt, 
      ...
      LOG.diagnostics_report_from(String.valueOf(taskAttempt.attemptId), String.valueOf(diagEvent.getDiagnosticInfo())).tag("methodCall").info();
