...
public class CommitterEventHandler extends AbstractService
  ...
  private static final /* LogLOG=LogFactory.getLog(CommitterEventHandler.class); */
  ...
  private class EventProcessor implements Runnable {
    ...
    protected void handleJobSetup(CommitterJobSetupEvent event) {
      try {
        ...
        LOG.job_setup_failed(e.toString()).warn();
    ...
    protected void handleJobCommit(CommitterJobCommitEvent event) {
      try {
        ...
        try {
          ...
          LOG.could_not_create_failure_file(e2.toString()).error();
        }
        /* LOG.error("Could not commit job",e) */
        LOG.could_not_commit_job(e.toString()).error();
        context.getEventHandler().handle(
    ...
    protected void handleJobAbort(CommitterJobAbortEvent event) {
      ...
      try {
        ...
        LOG.could_not_abort_job(e.toString()).warn();
      }
    ...
    protected void handleTaskAbort(CommitterTaskAbortEvent event) {
      try {
        ...
        LOG.task_cleanup_failed_for_attempt(String.valueOf(event.getAttemptID()), e.toString()).tag("methodCall").warn();
      }
      context.getEventHandler().handle(
