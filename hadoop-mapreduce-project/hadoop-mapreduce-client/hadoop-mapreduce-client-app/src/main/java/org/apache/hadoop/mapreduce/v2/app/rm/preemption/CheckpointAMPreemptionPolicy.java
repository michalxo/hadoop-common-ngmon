...
public class CheckpointAMPreemptionPolicy implements AMPreemptionPolicy {
  ...
  static final /* LogLOG=LogFactory.getLog(CheckpointAMPreemptionPolicy.class); */
  ...
  public void preempt(Context ctxt, PreemptionMessage preemptionRequests) {
    ...
    if (preemptionRequests != null) {
      ...
      if (cStrict != null
        ...
        LOG.strict_preemption_containers_kill(String.valueOf(preemptionRequests.getStrictContract().getContainers().size())).tag("methodCall").info();
        ...
        for (PreemptionContainer c :
          ...
          if (reqTask != null) {
            ...
            if (org.apache.hadoop.mapreduce.v2.api.records.TaskType.REDUCE
              ...
              LOG.preempting_running_task(reqCont.toString(), reqTask.toString()).info();
            } else {
              /* LOG.info("NOT preempting "+reqCont+" running task:"+reqTask) */
              LOG.not_preempting_running_task(reqCont.toString(), reqTask.toString()).info();
            }
          }
        }
      }
      ...
      if (cNegot != null
        ...
        LOG.negotiable_preemption_resourcereq_contai(String.valueOf(preemptionRequests.getContract().getResourceRequest().size()), String.valueOf(preemptionRequests.getContract().getContainers().size())).tag("methodCall").info();
        ...
        for (PreemptionResourceRequest rr : reqResources) {
          ...
          LOG.resourcerequest(reqRsrc.toString()).info();
          ...
          for (Container cont : listOfCont) {
            ...
            LOG.resourcerequest_satisfied_preempting(reqRsrc.toString(), reduceId.toString()).info();
          }
  ...
  public void handleCompletedContainer(TaskAttemptId attemptID){
    ...
    LOG.task_completed(attemptID.toString()).info();
    toBePreempted.remove(attemptID);
