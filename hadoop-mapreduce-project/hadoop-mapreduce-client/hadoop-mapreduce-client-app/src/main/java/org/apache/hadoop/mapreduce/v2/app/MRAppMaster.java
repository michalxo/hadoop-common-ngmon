...
public class MRAppMaster extends CompositeService {
  ...
  private static final /* LogLOG=LogFactory.getLog(MRAppMaster.class); */
  ...
  public MRAppMaster(ApplicationAttemptId applicationAttemptId,
    ...
    LOG.created_mrappmaster_for_application(applicationAttemptId.toString()).info();
  }
  ...
  protected void serviceInit(final Configuration conf) throws Exception {
    ...
    LOG.specific_max_attempts_for_application_at(maxAppAttempts, String.valueOf(appAttemptID.getApplicationId().getId()), String.valueOf(appAttemptID.getAttemptId()), isLastAMRetry).tag("methodCall").info();
    ...
    if ((numReduceTasks > 0 && 
      ...
      LOG.using_mapred_newapicommitter().info();
    }
    ...
    try {
      ...
      if(!stagingExists) {
        ...
        LOG.attempt_num_last_retry_because_staging(String.valueOf(appAttemptID.getAttemptId()), isLastAMRetry).tag("methodCall").info();
        errorHappenedShutDown = true;
        forcedState = JobStateInternal.ERROR;
        shutDownMessage = "Staging dir does not exist " + stagingDir;
        /* LOG.fatal(shutDownMessage) */
        LOG.shutdownmessage(shutDownMessage).fatal();
        ...
        LOG.attempt_num_last_retry_because_commit(String.valueOf(appAttemptID.getAttemptId()), isLastAMRetry).tag("methodCall").info();
        copyHistory = true;
        if (commitSuccess) {
  ...
  private OutputCommitter createOutputCommitter(Configuration conf) {
    ...
    LOG.outputcommitter_set_config(String.valueOf(conf.get("mapred.output.committer.class"))).tag("methodCall").info();
    ...
    LOG.outputcommitter(String.valueOf(committer.getClass().getName())).tag("methodCall").info();
    return committer;
  }
    ...
    try {
      if (!keepJobFiles(new JobConf(getConfig()))) {
        ...
        if (jobTempDir == null) {
          ...
          LOG.job_staging_directory_null().warn();
          return;
        }
        Path jobTempDirPath = new Path(jobTempDir);
        /* LOG.info("Deleting staging directory "+FileSystem.getDefaultUri(getConfig())+" "+jobTempDir) */
        LOG.deleting_staging_directory(String.valueOf(FileSystem.getDefaultUri(getConfig())), jobTempDir).tag("methodCall").info();
        fs.delete(jobTempDirPath, true);
      }
    } catch(IOException io) {
      /* LOG.error("Failed to cleanup staging dir "+jobTempDir,io) */
      LOG.failed_cleanup_staging_dir(jobTempDir, io.toString()).error();
    }
  }
    ...
    try {
      ...
      if ( !isLastAMRetry){
        if (((JobImpl)job).getInternalState() != JobStateInternal.REBOOT) {
          ...
          LOG.are_finishing_cleanly_this_last_retry().info();
          isLastAMRetry = true;
        }
      }
      ...
      LOG.calling_stop_for_all_services().info();
      ...
      if (isLastAMRetry) {
        ...
        if (getConfig().get(MRJobConfig.MR_JOB_END_NOTIFICATION_URL) != null) {
          try {
            ...
            LOG.job_end_notification_started_for_jobid(String.valueOf(job.getReport().getJobId())).tag("methodCall").info();
            ...
            LOG.job_end_notification_interrupted_for_job(String.valueOf(job.getReport().getJobId()), ie.toString()).tag("methodCall").warn();
          }
        }
      }
      ...
      LOG.graceful_stop_failed(t.toString()).warn();
    }

    //Bring the process down by force.
    //Not needed after HADOOP-7140
    /* LOG.info("Exiting MR AppMaster..GoodBye!") */
    LOG.exiting_appmaster_goodbye().info();
    sysexit();   
  }
  ...
  protected Speculator createSpeculator(Configuration conf, AppContext context) {
    ...
    try {
      ...
      LOG.cant_make_speculator_check(String.valueOf(MRJobConfig.MR_AM_JOB_SPECULATOR), ex.toString()).tag("methodCall").error();
    ...
    protected void serviceStop() throws Exception {
      try {
        if(isLastAMRetry) {
          ...
          LOG.skipping_cleaning_staging_dir_assuming_w().info();
        }
      } catch (IOException io) {
        /* LOG.error("Failed to cleanup staging dir: ",io) */
        LOG.failed_cleanup_staging_dir(io.toString()).error();
      }
      super.serviceStop();
    }
  }
  ...
  protected void serviceStart() throws Exception {
    ...
    if (!errorHappenedShutDown) {
      ...
      if (job.isUber()) {
        ...
        LOG.mrappmaster_uberizing_job_local_containe(String.valueOf(job.getID()), nmHost, nmPort).tag("methodCall").info();
      } else {
        ...
        LOG.mrappmaster_launching_normal_non_uberize(String.valueOf(job.getID())).tag("methodCall").info();
      }
  ...
  private void processRecovery() {
    ...
    if (recoveryEnabled && recoverySupportedByCommitter
      ...
      LOG.recovery_enabled_will_try_recover_from().info();
      try {
        parsePreviousJobHistory();
      } catch (IOException e) {
        /* LOG.warn("Unable to parse prior job history, aborting recovery",e) */
        LOG.unable_parse_prior_job_history_aborting(e.toString()).warn();
      ...
      LOG.will_not_try_recover_recoveryenabled_rec(recoveryEnabled, recoverySupportedByCommitter, numReduceTasks, shuffleKeyValidForRecovery, String.valueOf(appAttemptID.getAttemptId())).tag("methodCall").info();
  ...
  private static FSDataInputStream getPreviousJobHistoryStream(
    ...
    LOG.previous_history_file(historyFile.toString()).info();
  ...
  private void parsePreviousJobHistory() throws IOException {
    ...
    if (parseException != null) {
      ...
      LOG.got_error_parsing_job_history_file_ignor(parseException.toString()).info();
    }
    ...
    for (TaskInfo taskInfo : taskInfos.values()) {
      if (TaskState.SUCCEEDED.toString().equals(taskInfo.getTaskStatus())) {
        ...
        LOG.read_from_history_task(String.valueOf(TypeConverter.toYarn(taskInfo.getTaskId()))).tag("methodCall").info();
      }
    }
    /* LOG.info("Read completed tasks from history "+completedTasksFromPreviousRun.size()) */
    LOG.read_completed_tasks_from_history(String.valueOf(completedTasksFromPreviousRun.size())).tag("methodCall").info();
  ...
  private List<AMInfo> readJustAMInfos() {
    ...
    try {
      ...
      LOG.could_not_parse_old_history_file_will(e.toString()).warn();
    } finally {
      if (inputStream != null) {
  ...
  public static void main(String[] args) {
    try {
      ...
      if (systemPropsToLog != null) {
        ...
        LOG.systempropstolog(systemPropsToLog).info();
      }
      ...
      LOG.error_starting_mrappmaster(t.toString()).fatal();
  ...
  static class MRAppMasterShutdownHook implements Runnable {
    ...
    public void run() {
      ...
      LOG.mrappmaster_received_signal_signaling_rm().info();
  ...
  public void notifyIsLastAMRetry(boolean isLastAMRetry){
    if(containerAllocator instanceof ContainerAllocatorRouter) {
      ...
      LOG.notify_rmcommunicator_isamlastretry(isLastAMRetry).info();
    ...
    if(jobHistoryEventHandler != null) {
      ...
      LOG.notify_jheh_isamlastretry(isLastAMRetry).info();
  ...
  protected static void initAndStartAppMaster(final MRAppMaster appMaster,
    ...
    LOG.executing_with_tokens().info();
    for (Token<?> token : credentials.getAllTokens()) {
      /* LOG.info(token) */
      LOG.token(token.toString()).info();
    }
