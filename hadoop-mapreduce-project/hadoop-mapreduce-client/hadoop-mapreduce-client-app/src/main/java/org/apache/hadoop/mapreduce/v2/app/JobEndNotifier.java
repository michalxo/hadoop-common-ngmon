...
public class JobEndNotifier implements Configurable {
  ...
  private static final String JOB_STATUS = "$jobStatus";
    private static final MapreduceNamespace Log = LoggerFactory.getLogger(MapreduceNamespace.class, new SimpleLogger());
...
//              Log.info("Job end notification using proxy type \"" + proxyType + "\" hostname \"" + hostname + "\" and port \"" + port + "\"");
              Log.job_end_notification_using_proxy_type_ho(proxyType.toString(), hostname, port).info();
          } catch (NumberFormatException nfe) {
        /* Log.warn("Job end notification couldn't parse configured proxy's port "+portConf+". Not going to use a proxy") */
              Log.job_end_notification_couldnt_parse_confi(portConf).warn();
          }
      }
  ...
  protected boolean notifyURLOnce() {
    ...
    try {
      ...
      Log.job_end_notification_trying(urlToNotify.toString()).info();
      ...
      if(conn.getResponseCode() != HttpURLConnection.HTTP_OK) {
        ...
        Log.job_end_notification_failed_with_code_an(urlToNotify.toString(), String.valueOf(conn.getResponseCode()), String.valueOf(conn.getResponseMessage())).tag("methodCall").warn();
      }
      else {
        success = true;
        /* Log.info("Job end notification to "+urlToNotify+" succeeded") */
        Log.job_end_notification_succeeded(urlToNotify.toString()).info();
      }
    } catch(IOException ioe) {
      /* Log.warn("Job end notification to "+urlToNotify+" failed",ioe) */
      Log.job_end_notification_failed(urlToNotify.toString(), ioe.toString()).warn();
    }
    return success;
  }
  ...
  public void notify(JobReport jobReport)
    ...
    if (userUrl == null) {
      ...
      Log.job_end_notification_url_not_set().info();
      return;
    }
    ...
    try {
      ...
      Log.job_end_notification_couldnt_parse(userUrl, mue.toString()).warn();
      return;
    }
    ...
    while (numTries-- > 0 && !success) {
      ...
      Log.job_end_notification_attempts_left(numTries).info();
    ...
    if (!success) {
      ...
      Log.job_end_notification_failed_notify(urlToNotify.toString()).warn();
    } else {
      /* Log.info("Job end notification succeeded for "+jobReport.getJobId()) */
      Log.job_end_notification_succeeded_for(String.valueOf(jobReport.getJobId())).tag("methodCall").info();
    }
  }
}
