...
public class AppController extends Controller implements AMParams {
  private static final /* LogLOG=LogFactory.getLog(AppController.class); */
  ...
  public void tasks() {
    ...
    if (app.getJob() != null) {
      try {
        ...
        LOG.failed_render_tasks_page_with_task_for($(TASK_TYPE), $(JOB_ID), e.toString()).error();
  ...
  public void attempts() {
    ...
    if (app.getJob() != null) {
      try {
        ...
        LOG.failed_render_attempts_page_with_task_fo($(TASK_TYPE), $(JOB_ID), e.toString()).error();
        badRequest(e.getMessage());
      }
    }
  }
