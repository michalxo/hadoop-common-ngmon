...
public class DefaultSpeculator extends AbstractService implements
  ...
  private static final /* LogLOG=LogFactory.getLog(DefaultSpeculator.class); */
  ...
  static private TaskRuntimeEstimator getEstimator
    ...
    try {
      ...
      LOG.cant_make_speculation_runtime_extimator(ex.toString()).error();
...
/*   *************************************************************    */
              ...
              while (!stopped && !Thread.currentThread().isInterrupted()) {
                ...
                try {
                  ...
                  if (speculations > 0) {
                    ...
                    LOG.launched_speculations_sleeping_milliseco(speculations, wait).info();
                  }
                  ...
                  if (!stopped) {
                    ...
                    LOG.background_thread_returning_interrupted(e.toString()).error();
                  }
                  return;
                }
              }
  ...
  public void scanForSpeculations() {
    ...
    LOG.got_asked_run_debug_speculation_scan().info();
    // debug
  ...
  private synchronized void processSpeculatorEvent(SpeculatorEvent event) {
    switch (event.getType()) {
      ...
      case ATTEMPT_START:
      {
        ...
        LOG.attempt_start(String.valueOf(event.getTaskID())).tag("methodCall").info();
      ...
      case JOB_CREATE:
      {
        ...
        LOG.job_create(String.valueOf(event.getJobID())).tag("methodCall").info();
...
//  those jobs that need them.
  ...
  protected void addSpeculativeAttempt(TaskId taskID) {
    ...
    LOG.defaultspeculator_addspeculativeattempt_(taskID.toString()).info();
