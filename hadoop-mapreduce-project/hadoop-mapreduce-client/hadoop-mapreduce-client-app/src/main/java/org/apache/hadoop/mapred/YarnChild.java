...
class YarnChild {
  ...
  private static final /* LogLOG=LogFactory.getLog(YarnChild.class); */
  ...
  public static void main(String[] args) throws Throwable {
    ...
    LOG.pid(System.getenv().get("JVM_PID")).tag("methodCall").debug();
    Task task = null;
    ...
    try {
      ...
      for (int idle = 0; null == myTask; ++idle) {
        ...
        LOG.sleeping_for_before_retrying_again_got(sleepTimeMilliSecs).info();
      ...
      LOG.exception_running_child(StringUtils.stringifyException(exception)).tag("methodCall").warn();
      try {
        if (task != null) {
      ...
      LOG.error_running_child(StringUtils.stringifyException(throwable)).tag("methodCall").fatal();
      if (taskid != null) {
  ...
  private static void configureLocalDirs(Task task, JobConf job) throws IOException {
    ...
    LOG.for_child(String.valueOf(MRConfig.LOCAL_DIR), String.valueOf(job.get(MRConfig.LOCAL_DIR))).tag("methodCall").info();
  ...
  private static void configureTask(JobConf job, Task task,
    ...
    LOG.application_attempt_id(appAttemptId.toString()).debug();
    ...
    if (shuffleSecret == null) {
      ...
      LOG.shuffle_secret_missing_from_task_credent().warn();
