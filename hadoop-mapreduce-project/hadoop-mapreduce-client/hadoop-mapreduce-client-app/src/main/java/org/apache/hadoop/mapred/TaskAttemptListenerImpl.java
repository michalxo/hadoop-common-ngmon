...
public class TaskAttemptListenerImpl extends CompositeService 
  ...
  private static final /* LogLOG=LogFactory.getLog(TaskAttemptListenerImpl.class); */
  ...
  public boolean canCommit(TaskAttemptID taskAttemptID) throws IOException {
    ...
    LOG.commit_request_from(String.valueOf(taskAttemptID.toString())).tag("methodCall").info();
  ...
  public void commitPending(TaskAttemptID taskAttemptID, TaskStatus taskStatsu)
    ...
    LOG.commit_pending_state_update_from(String.valueOf(taskAttemptID.toString())).tag("methodCall").info();
    // An attempt is asking if it can commit its output. This can be decided
    // only by the task which is managing the multiple attempts. So redirect the
    // request there.
    org.apache.hadoop.mapreduce.v2.api.records.TaskAttemptId attemptID =
  ...
  public void preempted(TaskAttemptID taskAttemptID, TaskStatus taskStatus)
    ...
    LOG.preempted_state_update_from(String.valueOf(taskAttemptID.toString())).tag("methodCall").info();
  ...
  public void done(TaskAttemptID taskAttemptID) throws IOException {
    ...
    LOG.done_acknowledgement_from(String.valueOf(taskAttemptID.toString())).tag("methodCall").info();
  ...
  public void fatalError(TaskAttemptID taskAttemptID, String msg)
    ...
    LOG.task_exited(taskAttemptID.toString(), msg).fatal();
  ...
  public void fsError(TaskAttemptID taskAttemptID, String message)
    ...
    LOG.task_failed_due_fserror(taskAttemptID.toString(), message).fatal();
  ...
  public MapTaskCompletionEventsUpdate getMapCompletionEvents(
    ...
    LOG.mapcompletionevents_request_from_startin(String.valueOf(taskAttemptID.toString()), startIndex, maxEvents).tag("methodCall").info();
 ...
 throws IOException {
    ...
    LOG.diagnostics_report_from(String.valueOf(taskAttemptID.toString()), diagnosticInfo).tag("methodCall").info();
  ...
  public AMFeedback statusUpdate(TaskAttemptID taskAttemptID,
    ...
    if (getConfig().getBoolean(MRJobConfig.TASK_PREEMPTION, false)
      ...
      LOG.setting_preemption_bit_for_task_type(yarnAttemptID.toString(), String.valueOf(yarnAttemptID.getTaskId().getTaskType())).tag("methodCall").info();
    }
    ...
    if (taskStatus == null) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.ping_from(String.valueOf(taskAttemptID.toString())).tag("methodCall").debug();
      }
      return feedback;
    }
    ...
    LOG.progress_taskattempt(taskAttemptID.toString(), String.valueOf(taskStatus.getProgress())).tag("methodCall").info();
...
//    taskStatus.getIncludeCounters();
  ...
  public JvmTask getTask(JvmContext context) throws IOException {
    ...
    LOG.jvm_with_asked_for_task(jvmId.toString()).info();
    ...
    if (!jvmIDToActiveAttemptMap.containsKey(wJvmID)) {
      ...
      LOG.jvm_with_invalid_and_will_killed(jvmId.toString()).info();
      ...
      if (!launchedJVMs.contains(wJvmID)) {
        ...
        LOG.jvm_with_asking_for_task_before(jvmId.toString()).info();
      } else {
        ...
        LOG.jvm_with_given_task(jvmId.toString(), String.valueOf(task.getTaskID())).tag("methodCall").info();
