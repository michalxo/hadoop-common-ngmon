...
public abstract class RMContainerRequestor extends RMCommunicator {
  ...
  private static final /* LogLOG=LogFactory.getLog(RMContainerRequestor.class); */
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    LOG.nodeblacklistingenabled(nodeBlacklistingEnabled).info();
    ...
    LOG.maxtaskfailurespernode(maxTaskFailuresPerNode).info();
    ...
    LOG.blacklistdisablepercent(blacklistDisablePercent).info();
  }
  ...
  protected AllocateResponse makeRemoteRequest() throws IOException {
    ...
    if (ask.size() > 0 || release.size() > 0) {
      ...
      LOG.getresources_for_ask_release_newcontaine(applicationId.toString(), String.valueOf(ask.size()), String.valueOf(release.size()), String.valueOf(allocateResponse.getAllocatedContainers().size()), String.valueOf(allocateResponse.getCompletedContainersStatuses().size()), availableResources.toString(), clusterNmCount).tag("methodCall").info();
    }
    ...
    if (blacklistAdditions.size() > 0 || blacklistRemovals.size() > 0) {
      ...
      LOG.update_blacklist_for_blacklistadditions_(applicationId.toString(), String.valueOf(blacklistAdditions.size()), String.valueOf(blacklistRemovals.size())).tag("methodCall").info();
    }
  ...
  protected void computeIgnoreBlacklisting() {
    ...
    if (blacklistDisablePercent != -1
      ...
      if (clusterNmCount == 0) {
        ...
        LOG.knownnode_count_not_computing_ignoreblac().info();
        return;
      }
      ...
      if (val >= blacklistDisablePercent) {
        if (ignoreBlacklisting.compareAndSet(false, true)) {
          ...
          LOG.ignore_blacklisting_set_true_known_black(clusterNmCount, blacklistedNodeCount, val).info();
        ...
        if (ignoreBlacklisting.compareAndSet(true, false)) {
          ...
          LOG.ignore_blacklisting_set_false_known_blac(clusterNmCount, blacklistedNodeCount, val).info();
  ...
  protected void containerFailedOnHost(String hostName) {
    ...
    if (blacklistedNodes.contains(hostName)) {
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.host_already_blacklisted(hostName).debug();
      }
    ...
    LOG.failures_node(failures.toString(), hostName).info();
    if (failures >= maxTaskFailuresPerNode) {
      ...
      LOG.blacklisted_host(hostName).info();
  ...
  private void addResourceRequest(Priority priority, String resourceName,
    ...
    if (remoteRequests == null) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.added_priority(priority.toString()).debug();
      }
    }
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.addresourcerequest_applicationid_priorit(String.valueOf(applicationId.getId()), String.valueOf(priority.getPriority()), resourceName, String.valueOf(remoteRequest.getNumContainers()), String.valueOf(ask.size())).tag("methodCall").debug();
    }
  }
  ...
  private void decResourceRequest(Priority priority, String resourceName,
    ...
    if (reqMap == null) {
      ...
      if (LogGlobal.isDebugEnabled()) {
        ...
        LOG.not_decrementing_resource_not_present_re(resourceName).debug();
      }
      return;
    }
    ResourceRequest remoteRequest = reqMap.get(capability);
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.before_decresourcerequest_applicationid_(String.valueOf(applicationId.getId()), String.valueOf(priority.getPriority()), resourceName, String.valueOf(remoteRequest.getNumContainers()), String.valueOf(ask.size())).tag("methodCall").debug();
    }
    ...
    if (LogGlobal.isDebugEnabled()) {
      ...
      LOG.after_decresourcerequest_applicationid_p(String.valueOf(applicationId.getId()), String.valueOf(priority.getPriority()), resourceName, String.valueOf(remoteRequest.getNumContainers()), String.valueOf(ask.size())).tag("methodCall").info();
    }
  }
