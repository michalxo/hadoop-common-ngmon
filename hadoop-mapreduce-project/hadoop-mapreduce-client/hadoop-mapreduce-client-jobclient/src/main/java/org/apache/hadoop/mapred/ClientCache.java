...
public class ClientCache {
  ...
  private static final /* LogLOG=LogFactory.getLog(ClientCache.class); */
  ...
  public synchronized ClientServiceDelegate getClient(JobID jobId) {
    if (hsProxy == null) {
      try {
        ...
        LOG.could_not_connect_history_server(e.toString()).warn();
  ...
  protected MRClientProtocol instantiateHistoryProxy()
    ...
    LOG.connecting_historyserver(serviceAddr).debug();
    final YarnRPC rpc = YarnRPC.create(conf);
    /* LOG.debug("Connected to HistoryServer at: "+serviceAddr) */
    LOG.connected_historyserver(serviceAddr).debug();
