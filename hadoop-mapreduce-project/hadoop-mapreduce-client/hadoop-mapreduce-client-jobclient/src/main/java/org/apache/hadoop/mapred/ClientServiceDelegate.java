...
public class ClientServiceDelegate {
  private static final /* LogLOG=LogFactory.getLog(ClientServiceDelegate.class); */
  ...
  private MRClientProtocol getProxy() throws IOException {
    ...
    while (application == null
      ...
      if (application == null) {
        ...
        LOG.could_not_get_job_info_from_redirecting(jobId.toString()).info();
        return checkAndGetHSProxy(null, JobState.NEW);
      }
      try {
        if (application.getHost() == null || "".equals(application.getHost())) {
          /* LOG.debug("AM not assigned to Job. Waiting to get the AM ...") */
          LOG.not_assigned_job_waiting_get().debug();
          Thread.sleep(2000);

          /* LOG.debug("Application state is "+application.getYarnApplicationState()) */
          LOG.application_state(String.valueOf(application.getYarnApplicationState())).tag("methodCall").debug();
          application = rm.getApplicationReport(appId);
          continue;
          ...
          if (!amAclDisabledStatusLogged) {
            ...
            LOG.job_running_but_host_unknown_verify(jobId.toString()).info();
        ...
        if(!conf.getBoolean(MRJobConfig.JOB_AM_ACCESS_DISABLED, false)) {
          ...
          LOG.connecting(serviceAddr.toString()).debug();
          ...
          if (!amAclDisabledStatusLogged) {
            ...
            LOG.network_acl_closed_for_job_not(jobId.toString()).info();
            amAclDisabledStatusLogged = true;
          }
        ...
        LOG.could_not_connect_waiting_for_getting(serviceAddr.toString()).info();
        try {
          Thread.sleep(2000);
        } catch (InterruptedException e1) {
          /* LOG.warn("getProxy() call interruped",e1) */
          LOG.getproxy_call_interruped(e1.toString()).warn();
        ...
        LOG.getproxy_call_interruped(e.toString()).warn();
    ...
    if (application.getYarnApplicationState() == YarnApplicationState.FINISHED) {
      ...
      LOG.application_state_completed_finalapplica(String.valueOf(application.getFinalApplicationStatus().toString())).tag("methodCall").info();
  ...
  private MRClientProtocol checkAndGetHSProxy(
    ...
    if (null == historyServerProxy) {
      ...
      LOG.job_history_server_not_configured().warn();
  ...
  MRClientProtocol instantiateAMProxy(final InetSocketAddress serviceAddr)
    ...
    LOG.connecting_applicationmaster(serviceAddr.toString()).trace();
    ...
    LOG.connected_applicationmaster(serviceAddr.toString()).trace();
    return proxy;
  }
  ...
  private synchronized Object invoke(String method, Class argClass,
    ...
    while (maxClientRetry > 0) {
      ...
      try {
        ...
        LOG.failed_contact_history_for_job_retrying(jobId.toString(), String.valueOf(e.getTargetException())).tag("methodCall").debug();
        ...
        try {
          ...
          LOG.clientservicedelegate_invoke_call_interr(ie.toString()).warn();
          throw new YarnRuntimeException(ie);
        }
      } catch (Exception e) {
        /* LOG.debug("Failed to contact AM/History for job "+jobId+"  Will retry..",e) */
        LOG.failed_contact_history_for_job_will(jobId.toString(), e.toString()).debug();
        // Force reconnection by setting the proxy to null.
        realProxy = null;
        // RM shutdown
        maxClientRetry--;
