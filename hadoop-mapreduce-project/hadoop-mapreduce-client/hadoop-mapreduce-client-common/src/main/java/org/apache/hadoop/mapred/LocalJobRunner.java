...
public class LocalJobRunner implements ClientProtocol {
  public static final /* LogLOG=LogFactory.getLog(LocalJobRunner.class); */
  ...
  private class Job extends Thread implements TaskUmbilicalProtocol {
    ...
    protected class MapTaskRunnable extends RunnableWithThrowable {
      ...
      public void run() {
        try {
          ...
          LOG.starting_task(mapId.toString()).info();
          mapIds.add(mapId);
          ...
          LOG.finishing_task(mapId.toString()).info();
        } catch (Throwable e) {
    ...
    protected class ReduceTaskRunnable extends RunnableWithThrowable {
      ...
      public void run() {
        try {
          ...
          LOG.starting_task(reduceId.toString()).info();
          ...
          if (!Job.this.isInterrupted()) {
            ...
            LOG.finishing_task(reduceId.toString()).info();
          } else {
    ...
    protected synchronized ExecutorService createMapExecutor() {
      ...
      LOG.starting_mapper_thread_pool_executor().debug();
      /* LOG.debug("Max local threads: "+maxMapThreads) */
      LOG.max_local_threads(maxMapThreads).debug();
      /* LOG.debug("Map tasks to process: "+this.numMapTasks) */
      LOG.map_tasks_process(String.valueOf(this.numMapTasks)).tag("methodCall").debug();
    ...
    protected synchronized ExecutorService createReduceExecutor() {
      ...
      LOG.starting_reduce_thread_pool_executor().debug();
      /* LOG.debug("Max local threads: "+maxReduceThreads) */
      LOG.max_local_threads(maxReduceThreads).debug();
      /* LOG.debug("Reduce tasks to process: "+this.numReduceTasks) */
      LOG.reduce_tasks_process(String.valueOf(this.numReduceTasks)).tag("methodCall").debug();
    ...
    private void runTasks(List<RunnableWithThrowable> runnables,
      ...
      try {
        ...
        LOG.waiting_for_tasks(taskType).info();
      ...
      LOG.task_executor_complete(taskType).info();
    ...
    public void run() {
      ...
      try {
        ...
        LOG.failed_createoutputcommitter(e.toString()).info();
        return;
      }
      ...
      try {
        ...
        try {
          ...
          LOG.error_cleaning_job(id.toString()).info();
        }
        status.setCleanupProgress(1.0f);
        if (killed) {
          this.status.setRunState(JobStatus.KILLED);
        ...
        /* LOG.warn(id,t) */
        LOG.idexception(id.toString(), t.toString()).warn();
        ...
        try {
          ...
          LOG.error_cleaning(id.toString(), e.toString()).warn();
        }
      }
    }
    ...
    public synchronized AMFeedback statusUpdate(TaskAttemptID taskId,
      ...
      LOG.taskstatusgetsta(String.valueOf(taskStatus.getStateString())).tag("methodCall").info();
    ...
    public void reportNextRecordRange(TaskAttemptID taskid, 
      ...
      LOG.task_reportednextrecordrange(taskid.toString(), range.toString()).info();
    }
    ...
    throws IOException {
      ...
      LOG.fserror_from_task(message, taskId.toString()).fatal();
    }

    @Override
    public void shuffleError(TaskAttemptID taskId, String message) throws IOException {
      /* LOG.fatal("shuffleError: "+message+"from task: "+taskId) */
      LOG.shuffleerror_from_task(message, taskId.toString()).fatal();
    }
    
    public synchronized void fatalError(TaskAttemptID taskId, String msg) 
    throws IOException {
      /* LOG.fatal("Fatal: "+msg+"from task: "+taskId) */
      LOG.fatal_from_task(msg, taskId.toString()).fatal();
    }
  ...
  static void setupChildMapredLocalDirs(Task t, JobConf conf) {
    ...
    LOG.for_child(String.valueOf(MRConfig.LOCAL_DIR), childMapredLocalDir.toString()).tag("methodCall").debug();
