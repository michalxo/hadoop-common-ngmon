...
public class FileNameIndexUtils {
  ...
  private static final /* LogLOG=LogFactory.getLog(FileNameIndexUtils.class); */
  ...
  public static JobIndexInfo getIndexInfo(String jhFileName) throws IOException {
    ...
    try {
      try {
        ...
        LOG.unable_parse_submit_time_from_job(jhFileName, e.toString()).warn();
      }
      ...
      try {
        ...
        LOG.unable_parse_finish_time_from_job(jhFileName, e.toString()).warn();
      }
      ...
      try {
        ...
        LOG.unable_parse_num_maps_from_job(jhFileName, e.toString()).warn();
      }
      ...
      try {
        ...
        LOG.unable_parse_num_reduces_from_job(jhFileName, e.toString()).warn();
      }
      ...
      try{
        ...
        /* LOG.warn("Unable to parse launch time from job history file "+jhFileName+" : "+e) */
        LOG.unable_parse_launch_time_from_job(jhFileName, e.toString()).warn();
      }
    } catch (IndexOutOfBoundsException e) {
      /* LOG.warn("Parsing job history file with partial data encoded into name: "+jhFileName) */
      LOG.parsing_job_history_file_with_partial(jhFileName).warn();
    }
